...
type AthleteForm = {
  ...
  education: string; technicalLevel: string; athletePosition: string; healthStatus: string; bestResult: string;
...
const blankForm: AthleteForm = {
  ...
  emergencyContact: '', emergencyPhone: '', education: '', technicalLevel: '', athletePosition: '', healthStatus: '健康',
...
export function AthleteManagementPage({
  ...
  const [bulkForm, setBulkForm] = useState({ technicalLevel: '', athletePosition: '', healthStatus: '', currentEvent: '', athleteStatus: '', trainingPhase: '' });
  ...
  const filtered = useMemo(() => athletes.filter((athlete) => {
    ...
    return [athlete.name, athlete.project, athlete.team, athlete.coaches, athlete.technicalLevel, athlete.athletePosition, athlete.currentEvent, athlete.region, athlete.city]
  ...
  return (
    <div className="page-content athlete-management-page">
      ...
      {canManage && <section className={`athlete-batch-bar ${selectedIds.size ? 'visible' : ''}`}><span>已选择 <strong>{selectedIds.size}</strong> 名运动员</span><button onClick={() => { setBulkForm({ technicalLevel: '', athletePosition: '', healthStatus: '', currentEvent: '', athleteStatus: '', trainingPhase: '' }); setBulkOpen(true); }} disabled={!selectedIds.size}><PencilLine size={15} />批量修改</button><button className="danger" onClick={() => void deleteSelected()} disabled={!selectedIds.size}><Trash2 size={15} />批量删除</button></section>}
