...
function TrainingVolumePage({ sessions, days }: { sessions: Session[]; days: ReturnType<typeof aggregateDays> }) {
  ...
  return <><div className="special-metrics"><MetricCard icon={<CalendarDays/>} label="专项训练次数" value={sessions.length} unit="场" change={`${days.length} 个有效训练日`}/><MetricCard icon={<Clock3/>} label="专项训练时间" value={round(totalDuration / 60, 1)} unit="h" change={`日均 ${round(averageDailyDuration)} min`} tone="blue"/><MetricCard icon={<Route/>} label="专项训练量" value={round(totalDistance, 1)} unit="km" change="按训练距离累计" tone="green"/><MetricCard icon={<TimerReset/>} label="单次训练时间" value={round(averageDuration)} unit="min" change="按实际训练记录计算" tone="purple"/></div><div className="special-grid"><SectionCard title="专项训练量趋势" note="公里 · 按日" className="span-7"><TrendChart data={days} metric="distance" color="#347fe5"/></SectionCard><SectionCard title="专项训练时间趋势" note="小时 · 按日" className="span-5"><TrendChart data={days} metric="duration"/></SectionCard><SectionCard title="训练量类别构成" note="按训练距离统计" className="span-4"><TypeDonut sessions={sessions} valueKey="distance"/></SectionCard><SectionCard title="运动员训练时间对比" note="按累计训练时长排序" className="span-8"><AthleteTable sessions={sessions} metric="duration"/></SectionCard><SectionCard title="训练量与时间诊断" note="当前筛选周期" className="span-12"><InsightList items={[`当前周期完成 ${sessions.length} 场专项训练，覆盖 ${days.length} 个有效训练日。`,`累计训练量 ${round(totalDistance, 1)} km，累计训练时间 ${round(totalDuration / 60, 1)} 小时。`,`单次训练平均 ${round(averageDuration)} 分钟；建议结合“负荷分析”同步复核强度与恢复情况。`]}/></SectionCard></div></>;
}
...
export function SpecialTestsPage({ records, project, from, to, loading, section, onSectionChange }: Props) {
  ...
  const analysisTabs: Array<{ key: SpecialPageKey; label: string }> = [{ key: 'special-volume', label: '训练量' }, { key: 'special-time', label: '时间' }, { key: 'special-distance', label: '距离' }, { key: 'special-load', label: '负荷' }];
