...
export function StrengthProfileModule({ athlete, user }: Props) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        await exportPdfSheets(pdfRef.current, `${athlete.name}_体能测试档案_${selected.testDate}`, '齐总');
      } catch (error) {
  ...
  return (
    <section className="strength-module archive-profile-module">
      <header className="strength-module-heading">
        <div className="strength-module-title"><ClipboardList size={22} /><span><small>PHYSICAL TESTING</small><strong>体能测试档案</strong></span></div>
        <div className="strength-module-actions">
          ...
          <button className="primary-button" onClick={() => { if (selected) { setPdfBusy(true); setPdfError(''); setPdfOpen(true); } }} disabled={!selected || pdfBusy}>{pdfBusy ? <LoaderCircle className="spin" size={16} /> : <Download size={16} />}{pdfBusy ? '生成中…' : '导出体能测试档案'}</button>
        </div>
      </header>
      ...
      {loading ? <div className="strength-loading"><LoaderCircle className="spin" /><span>正在读取体能测试档案…</span></div> : selected ? <>
      ...
      </> : <div className="strength-empty"><ClipboardList size={34} /><strong>暂无体能测试档案数据</strong><p>{canEdit ? '录入第一次测试后，系统会生成评分表与雷达图。' : '教练录入测试后，你可以在这里查看和导出体能测试档案。'}</p>{canEdit && <div className="archive-empty-actions"><button className="primary-button" onClick={() => openEditor('new')}><Plus size={16} />录入第一次测试</button></div>}</div>}
...
function ArchiveSheet({ athlete, test, previous, variant }: { athlete: Athlete; test: StrengthTest; previous: StrengthTest | null; variant: 'web' | 'pdf' }) {
  ...
  return <div className={`strength-poster strength-poster-${variant} archive-sheet archive-sheet-${variant}`}>
    <header className="archive-sheet-header">
      ...
      <h2><span>{year}年{athlete.project}体能训练营</span>体能测试档案</h2>
