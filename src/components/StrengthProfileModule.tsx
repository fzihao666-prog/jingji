...
type ArchiveFactor = {
  ...
  lowerIsBetter?: StrengthMetricKey[];
};
...
const archiveFactors: ArchiveFactor[] = [
  {
    key: 'composition',
    label: '身体成分',
    metricKeys: ['bodyFatPct'],
    lowerIsBetter: ['bodyFatPct'],
  },
  ...
  { key: 'maximum', label: '最大力量', metricKeys: ['squatKg', 'deadliftKg'] },
  {
    key: 'core',
    label: '核心稳定',
    metricKeys: ['frontPlankSec', 'leftPlankSec', 'rightPlankSec'],
  },
  {
    key: 'endurance',
    label: '下肢耐力',
    metricKeys: ['leftSingleLegSquatReps', 'rightSingleLegSquatReps'],
  },
...
export function StrengthProfileModule({ athlete, user }: Props) {
  ...
  const loadTests = async (preferredId?: number) => {
    ...
    try {
      ...
      setSelectedId(
        preferredId && result.tests.some((test) => test.id === preferredId)
          ? preferredId
          : result.tests[0]?.id || null
      );
    } catch (error) {
      setLoadError(error instanceof Error ? error.message : '体能测试档案数据加载失败。');
    } finally {
      setLoading(false);
    }
  };
    ...
    const run = async () => {
      ...
      try {
        await exportPdfSheets(
          pdfRef.current,
          `${athlete.name}_体能测试档案_${selected.testDate}`,
          '齐总'
        );
      } catch (error) {
        setPdfError(error instanceof Error ? error.message : '体能测试档案PDF生成失败。');
      } finally {
        if (!cancelled) {
          setPdfOpen(false);
          setPdfBusy(false);
  ...
  const closeEditor = () => {
    ...
    if (
      editorMode === 'new' &&
      (Object.values(formMetrics).some(Boolean) ||
        Object.values(formTargets).some(Boolean) ||
        notes.trim())
    ) {
      persistDraft();
  ...
  const save = async (event: FormEvent) => {
    ...
    if (
      editorMode === 'new' &&
      tests.some((test) => test.testDate === testDate) &&
      !window.confirm(`${testDate}已有测试记录，继续保存会更新该日期的数据。是否继续？`)
    )
      return;
    setSaveBusy(true);
    setSaveError('');
    try {
      ...
      setEditorOpen(false);
      setDataNotice(
        editorMode === 'new' ? '测试数据已保存，档案和雷达图已更新。' : '本次测试数据已更新。'
      );
      await loadTests(result.id);
    } catch (error) {
      setSaveError(error instanceof Error ? error.message : '体能测试档案数据保存失败。');
    } finally {
      setSaveBusy(false);
    }
  };
  ...
  const previewStandards = useMemo(
    () => ({ ...previewBaseline?.targets, ...previewTargets }),
    [previewBaseline, previewTargets]
  );
  const previewBeforeScores = useMemo(
    () => archiveFactors.map((factor) => factorScore(factor, previewBaseline, previewStandards)),
    [previewBaseline, previewStandards]
  );
  const previewAfterScores = useMemo(
    () =>
      archiveFactors.map((factor) => factorScoreValues(factor, previewMetrics, previewStandards)),
    [previewMetrics, previewStandards]
  );
    ...
    <section className="strength-module archive-profile-module">
      <header className="strength-module-heading">
        <div className="strength-module-title">
          ...
          <span>
            <small>PHYSICAL TESTING</small>
            <strong>体能测试档案</strong>
          </span>
        </div>
        <div className="strength-module-actions">
          {tests.length > 1 && (
            <label className="strength-history-select">
              <History size={15} />
              <select
                aria-label="档案测试日期"
                value={selected?.id || ''}
          ...
          {canEdit && (
            <button
              className="secondary-button archive-new-test-button"
          ...
          {canEdit && selected && (
            <button className="secondary-button" onClick={() => openEditor('edit')}>
              <PencilLine size={16} />
              编辑本次数据
            </button>
          )}
      ...
      {dataNotice && (
        <div className="archive-import-notice">
          <Save size={16} />
          {dataNotice}
        </div>
      )}
      {loading ? (
        <div className="strength-loading">
          ...
          <span>正在读取体能测试档案…</span>
        </div>
      ) : selected ? (
        <>
          <div className="archive-sheet-scroll">
            <ArchiveSheet athlete={athlete} test={selected} previous={previous} variant="web" />
          </div>
          {athlete.project === '激流' && (
            <SlalomChampionComparison athlete={athlete} test={selected} />
          )}
        </>
      ) : (
        <div className="strength-empty">
          <ClipboardList size={34} />
          <strong>暂无体能测试档案数据</strong>
          <p>
            {canEdit
              ? '录入第一次测试后，系统会生成评分表与雷达图。'
              : '教练录入测试后，你可以在这里查看和导出体能测试档案。'}
          </p>
          {canEdit && (
      ...
      {editorOpen && (
        <div
          className="modal-backdrop strength-editor-backdrop"
          role="presentation"
          onMouseDown={(event) => {
        ...
        >
          <section
            className="strength-editor-modal archive-entry-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header>
              <div>
                <span>{editorMode === 'new' ? 'NEW FITNESS TEST' : 'EDIT FITNESS TEST'}</span>
                <h2 id="strength-editor-title">
                  {athlete.name} · {editorMode === 'new' ? '录入新测试' : '编辑本次数据'}
                </h2>
              </div>
              <button className="icon-button" onClick={closeEditor} aria-label="关闭">
                <X size={20} />
              </button>
            </header>
            <form onSubmit={save}>
              <div className="archive-entry-meta">
                <div className="archive-entry-athlete">
                  <span>当前运动员</span>
                ...
                <label className="archive-entry-note">
                  <span>测试说明</span>
                  <input
                    value={notes}
                    onChange={(event) => setNotes(event.target.value)}
                    maxLength={500}
                    placeholder="例如：阶段体能后测"
                  />
                </label>
                <button
                  type="button"
                  className="secondary-button copy-target-button"
                  disabled={!selected || !Object.keys(selected.targets).length}
                  onClick={copyPreviousTargets}
                >
                  <Copy size={15} />
                  沿用上次目标
                </button>
              ...
              <div className="archive-entry-layout">
                <section className="archive-entry-fields">
                  <div className="strength-entry-head">
                    <span>测试项目</span>
                    <span>实测值</span>
                    <span>满分标准／教练目标</span>
                  </div>
                  <div className="strength-entry-scroll">
                    {(Object.keys(STRENGTH_GROUPS) as StrengthGroupKey[]).map((groupKey) => {
                      ...
                      const metrics = STRENGTH_METRICS.filter(
                        (metric) =>
                          metric.group === groupKey &&
                          (!metric.projects || metric.projects.includes(athlete.project))
                      );
                      if (!metrics.length) return null;
                      return (
                        <section className="strength-entry-group" key={groupKey}>
                          <h3 style={{ '--metric-color': group.color } as CSSProperties}>
                            <i />
                            {groupKey === 'morphology' ? '基本信息' : group.label}
                          </h3>
                          {metrics.map((metric) => (
                            <div className="strength-entry-row" key={metric.key}>
                              ...
                              <div>
                                <input
                                  ...
                                  min={metric.min}
                                  max={metric.max}
                                  value={formMetrics[metric.key]}
                                  onChange={(event) => {
                                    setFormMetrics((current) => ({
                                      ...current,
                              ...
                              <div className={!metric.targetEnabled ? 'target-disabled' : ''}>
                                <input
                                  aria-label={`${metric.label}教练目标`}
                                  type="number"
                                  step="0.1"
                                  min={metric.min}
                                  max={metric.max}
                                  disabled={!metric.targetEnabled}
                                  value={formTargets[metric.key]}
                                  onChange={(event) => {
                                    setFormTargets((current) => ({
                                      ...current,
                                      [metric.key]: event.target.value,
                                    }));
                                    setDraftNotice('');
                                  }}
                                  placeholder={metric.targetEnabled ? '未设置' : '不评价'}
                                />
                                <span>{metric.targetEnabled ? metric.unit : '—'}</span>
                              </div>
                            </div>
                          ))}
                        </section>
                ...
                <aside className="archive-live-preview">
                  <header>
                    <span>REAL-TIME PROFILE</span>
                    <strong>档案实时预览</strong>
                  </header>
                  <div className="archive-preview-scope">
                    <span>{editorMode === 'new' ? '新增测试' : '当前测试'}</span>
                    <strong>{testDate}</strong>
                    <small>
                      {previewBaseline ? `对比基线 ${previewBaseline.testDate}` : '暂无历史基线'}
                    </small>
                  </div>
                  <div className="archive-preview-stats">
                    <div>
                      ...
                      <strong>
                        {previewMetricCount}
                        <small>/{availableMetricCount}项</small>
                      </strong>
                    </div>
                    <div>
                      <span>当前总分</span>
                    ...
                    <div>
                      <span>目标值</span>
                      <strong>
                        {Object.keys(previewStandards).length}
                        <small>项</small>
                      </strong>
                    </div>
                  </div>
                  <ArchiveRadar
                    ...
                    before={previewBeforeScores}
                    after={previewAfterScores}
                  />
                  <p className="archive-preview-rule">
                    <ShieldCheck size={15} />
                    空白项目保持“未测试”，不会记为0分；雷达图随输入实时更新。
                  </p>
                </aside>
              </div>
              ...
              <footer>
                <p>姓名、项目、队伍和教练在这里保持只读；运动员账号没有录入权限。</p>
                <button type="button" className="secondary-button" onClick={closeEditor}>
                  {editorMode === 'new' ? '稍后继续' : '取消'}
                </button>
                {editorMode === 'new' && (
                  <button type="button" className="secondary-button" onClick={persistDraft}>
                    <Save size={16} />
                    保存草稿
                  </button>
                )}
                <button className="primary-button" disabled={saveBusy}>
                  {saveBusy ? (
                    <>
      ...
      {pdfOpen && selected && (
        <div className="pdf-export-stage" ref={pdfRef} aria-hidden="true">
          <article className="personal-pdf-sheet strength-pdf-sheet">
            <ArchiveSheet athlete={athlete} test={selected} previous={previous} variant="pdf" />
          </article>
        </div>
      )}
    </section>
...
function ArchiveSheet({
  ...
  return (
    <div
      className={`strength-poster strength-poster-${variant} archive-sheet archive-sheet-${variant}`}
    >
      <header className="archive-sheet-header">
        <div className="archive-institute">
          <strong>冠蒂本</strong>
          <span>训练数据中心</span>
        </div>
        <h2>
        ...
        <div className="archive-brand">
          <BrandLogo variant="full" className="print" />
          <small>测试日期：{formatDate(test.testDate)}</small>
          <small>地点：冠蒂本训练中心</small>
        </div>
      </header>
      ...
      <section className="archive-basic-grid">
        ...
        <ArchiveBasic
          label="位置/号位"
          english="Position"
          value={athlete.athletePosition || '未录入'}
        />
        <ArchiveBasic
          label="身高"
          english="Height"
          value={metricBasicValue(test, 'heightCm', athlete.heightCm)}
        />
        <ArchiveBasic
          label="体重"
          english="Weight"
          value={metricBasicValue(test, 'weightKg', athlete.weightKg)}
        />
        <ArchiveBasic
          label="体脂率"
          english="Body fat ratio"
          value={metricBasicValue(test, 'bodyFatPct', athlete.bodyFatPct)}
        />
        <ArchiveBasic
          label="训练年限"
          english="Training years"
          value={metricBasicValue(test, 'trainingYears')}
        />
      </section>
      <h3 className="archive-section-title">多要素雷达图</h3>
      <div className="archive-score-summary">
        <span>
          前测总分：<strong>{beforeTotal === null ? '—' : `${beforeTotal}分`}</strong>
        </span>
        <span>
          后测总分：<strong>{afterTotal === null ? '—' : `${afterTotal}分`}</strong>
        </span>
        {beforeTotal !== null && afterTotal !== null && (
          <span className={afterTotal >= beforeTotal ? 'improved' : 'declined'}>
            {afterTotal >= beforeTotal ? '▲' : '▼'} {Math.abs(afterTotal - beforeTotal)}分
          </span>
        )}
      </div>
      <section className="archive-assessment-top">
        <ArchiveScoreTable
          factors={archiveFactors.slice(0, 4)}
          scoreOffset={0}
          before={previous}
          after={test}
          standards={standards}
      ...
      <section className="archive-assessment-bottom">
        <ArchiveScoreTable
          factors={archiveFactors.slice(4)}
          scoreOffset={4}
          before={previous}
          after={test}
          standards={standards}
      ...
      <footer className="archive-sheet-note">
        <span>评分说明：单项按实测值与满分标准／教练目标折算，满分10分；未测试不记0分。</span>
        <span>数据更新：{test.updatedBy}</span>
      </footer>
    </div>
  );
}
...
function ArchiveScoreTable({
  ...
  return (
    <div className={`archive-score-table-wrap ${detail ? 'detail' : 'primary'}`}>
      <table className="archive-score-table">
        <thead>
          <tr>
            <th rowSpan={2}>体能要素</th>
            <th rowSpan={2}>检测指标</th>
            <th colSpan={2}>实测值 Result</th>
            <th rowSpan={2}>
              满分标准
              <br />
              Target
            </th>
            <th colSpan={2}>评分 Score</th>
          </tr>
          <tr>
            <th>前测</th>
            <th>后测</th>
          ...
          {factors.map((factor, localIndex) => {
            ...
            return (
              <tr key={factor.key}>
                <th>
                  {factor.label}
                  <small>{factor.key.toUpperCase()}</small>
                </th>
                <td>
                  {factor.metricKeys.map((key) => (
                    <span key={key}>{STRENGTH_METRIC_MAP[key].label}</span>
                  ))}
                </td>
                <td>{factorValues(factor, before?.metrics)}</td>
                <td>{factorValues(factor, after.metrics)}</td>
                <td>{factorValues(factor, standards)}</td>
                <td className={scoreClass(beforeScore)}>{beforeScore ?? '—'}</td>
                <td className={scoreClass(afterScore)}>{afterScore ?? '—'}</td>
              </tr>
            );
          })}
        </tbody>
...
function ArchiveRadar({
  ...
  const polygon = (values: Array<number | null>) =>
    values
      .map((value, index) =>
        radarPoint(index, value ?? 0, factors.length, centerX, centerY, radius)
      )
      .join(' ');
  return (
    <figure className="archive-radar">
      <svg viewBox="0 0 340 310" role="img" aria-label="前测与后测多要素雷达图">
        {[2, 4, 6, 8, 10].map((level) => (
          <polygon
            key={level}
            className={level === 10 ? 'radar-ring outer' : 'radar-ring'}
            points={polygon(factors.map(() => level))}
          />
        ))}
        {factors.map((factor, index) => {
          const [x, y] = radarPoint(index, 10, factors.length, centerX, centerY, radius);
          ...
          return (
            <g key={factor.key}>
              ...
              <text
                ...
                y={labelY}
                textAnchor={
                  labelX < centerX - 8 ? 'end' : labelX > centerX + 8 ? 'start' : 'middle'
                }
        ...
        <polygon className="radar-before" points={polygon(before)} />
        <polygon className="radar-after" points={polygon(after)} />
        {after.map((value, index) => {
          const [x, y] = radarPoint(index, value ?? 0, factors.length, centerX, centerY, radius);
          return <circle key={factors[index].key} cx={x} cy={y} r="2.7" />;
        })}
      </svg>
      <figcaption>
        <span>
...
function factorValues(factor: ArchiveFactor, values?: StrengthMetricValues) {
  return (
    <>
      {factor.metricKeys.map((key) => {
        const metric = STRENGTH_METRIC_MAP[key];
        const value = values?.[key];
        return (
          <span key={key}>
            {typeof value === 'number' ? `${formatNumber(value, 1)}${metric.unit}` : '—'}
          </span>
        );
      })}
    </>
...
function factorScore(
  factor: ArchiveFactor,
  test: StrengthTest | null,
  standards: StrengthMetricValues
) {
  return test ? factorScoreValues(factor, test.metrics, standards) : null;
}

function factorScoreValues(
  factor: ArchiveFactor,
  metrics: StrengthMetricValues,
  standards: StrengthMetricValues
) {
  const scores = factor.metricKeys.flatMap((key) => {
    ...
    if (typeof value !== 'number' || typeof target !== 'number' || value <= 0 || target <= 0)
      return [];
...
function totalScore(scores: Array<number | null>) {
  const available = scores.filter((score): score is number => score !== null);
  return available.length ? available.reduce((sum, score) => sum + score, 0) : null;
}
...
function metricBasicValue(test: StrengthTest, key: StrengthMetricKey, fallback?: number | null) {
  const metric = STRENGTH_METRIC_MAP[key];
  const value = test.metrics[key] ?? fallback;
  return typeof value === 'number' ? `${formatNumber(value, 1)}${metric.unit}` : '未测试';
}
function blankForm() {
...
function importableMetrics(project: string) {
  return STRENGTH_METRICS.filter((metric) => !metric.projects || metric.projects.includes(project));
}

function archiveDraftKey(athleteId: number) {
  return `jingji-personal-archive-draft-${athleteId}`;
}
...
function readArchiveDraft(athleteId: number): ArchiveDraft | null {
  try {
    ...
    return {
      testDate: /^\d{4}-\d{2}-\d{2}$/.test(parsed.testDate || '')
        ? parsed.testDate!
        : toIsoDate(new Date()),
