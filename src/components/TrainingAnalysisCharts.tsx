...
export function TrainingVolumeDashboard({ data, physiology, from, to }: { data: TrainingAnalytics; physiology: PhysiologyHeatmap; from: string; to: string }) {
  ...
  const trainingLoadDays: Array<{ date: string; specialLoad: number | null; physicalLoad: number | null }> = [];
  for (let date = from; date <= to; date = addDays(date, 1)) {
    const row = loadByDate.get(date);
    trainingLoadDays.push({ date, specialLoad: row?.specialLoad ?? null, physicalLoad: row?.physicalLoad ?? null });
  }
  ...
  return <div className="training-analytics-dashboard">
    <section className="training-analytics-summary" aria-label="训练量统计核心摘要">
      ...
      <article><span>测试</span><strong>{value(summary.testDurationMin === null ? null : summary.testDurationMin / 60)}<small>{summary.testDurationMin === null ? '' : ' h'}</small></strong><em>{summary.testDurationMin === null ? '未录入测试时长' : '按队伍测试日去重累计'}</em></article>
    ...
    <section className="training-analytics-grid" aria-label="训练量统计分析图表">
      ...
      <article className="training-analytics-chart training-analytics-load-trend"><header><div><span>TRAINING LOAD</span><h3>训练负荷分析</h3></div><small>sRPE · AU</small></header><div className="training-analytics-canvas">{trainingLoadDays.some((row) => row.specialLoad !== null || row.physicalLoad !== null) ? <ResponsiveContainer width="100%" height="100%"><ComposedChart data={trainingLoadDays} margin={{ top: 12, right: 12, left: 6, bottom: 0 }}><CartesianGrid stroke="#dce7e9" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="date" tickFormatter={(date) => String(date).slice(5).replace('-', '/')} tick={{ fontSize: 9, fill: '#62767d' }} axisLine={false} tickLine={false}/><YAxis label={{ value: 'sRPE（AU）', angle: -90, position: 'insideLeft', fill: '#62767d', fontSize: 9 }} tick={{ fontSize: 9, fill: '#62767d' }} axisLine={false} tickLine={false}/><Tooltip content={({ active, payload, label }) => {
        const row = payload?.[0]?.payload as typeof trainingLoadDays[number] | undefined;
      ...
      }}/><Legend wrapperStyle={{ fontSize: 10 }}/><Line type="monotone" dataKey="specialLoad" name="专项训练 sRPE" stroke="#0b7f7a" strokeWidth={2.6} dot={{ r: 2.5, fill: '#fff', stroke: '#0b7f7a', strokeWidth: 2 }} activeDot={{ r: 5 }} connectNulls={false}/><Line type="monotone" dataKey="physicalLoad" name="体能训练 sRPE" stroke="#d59125" strokeWidth={2.6} dot={{ r: 2.5, fill: '#fff', stroke: '#d59125', strokeWidth: 2 }} activeDot={{ r: 5 }} connectNulls={false}/></ComposedChart></ResponsiveContainer> : <TrainingAnalyticsEmpty text="暂无专项或体能训练负荷数据" />}</div></article>
