...
export function InjuryRecoveryModule({ athlete, user, asOfDate }: Props) {
  ...
  useEffect(() => {
    ...
    setMessage('');
    api
      .injuryRecords(athlete.id)
      .then(({ records: nextRecords }) => {
        if (!cancelled) setRecords(nextRecords);
      })
      .catch((error) => {
        if (!cancelled) setMessage(error instanceof Error ? error.message : '伤病记录读取失败。');
      })
      .finally(() => {
    ...
    <section className="injury-recovery-module">
      <header className="injury-module-heading">
        <div className="injury-title-lockup">
          ...
          <div>
            <small>INJURY & RECOVERY</small>
            <h2>伤病与恢复</h2>
            <p>记录当前状态、训练限制和恢复进程</p>
          </div>
        </div>
        <button type="button" className="secondary-button" onClick={openDialog}>
          <Plus size={16} />
          {isAthlete ? '提交疼痛反馈' : '新增伤病记录'}
        </button>
      </header>

      {loading ? (
        <div className="injury-module-loading">
        ...
        <div className="injury-module-body">
          <div
            className="injury-current-panel"
            style={{ '--injury-color': currentMeta.color } as CSSProperties}
          >
            ...
            {current ? (
              <>
                <div className="injury-current-name">
                  ...
                  <small>
                    {sideLabels[current.side]} · {current.bodyPart} · 疼痛 {current.painScore}/10
                  </small>
                </div>
                <div className="injury-current-review">
                  ...
                  <span>
                    <small>复查安排</small>
                    <strong>
                      {current.reviewDate ? formatDate(current.reviewDate) : '暂未设置'}
                    </strong>
                    {reviewNotice && <em>{reviewNotice}</em>}
                  </span>
                </div>
              </>
            ) : (
              <div className="injury-clear-state">
                <ShieldCheck size={27} />
                <span>建立第一条记录后，可持续追踪恢复变化。</span>
              </div>
            )}
          </div>
              ...
              <article>
                ...
                <p>
                  {current.restrictions ||
                    (current.recordType === 'feedback' ? '等待教练确认训练限制' : '未设置限制')}
                </p>
              </article>
              <article>
                <span>恢复安排</span>
                <p>
                  {current.rehabPlan ||
                    (current.recordType === 'feedback' ? '等待教练制定恢复安排' : '未填写恢复计划')}
                </p>
              </article>
              <article>
                <span>补充说明</span>
                <p>{current.note || '无补充说明'}</p>
              </article>
            </div>
          )}

          <div className="injury-history-panel">
            <div className="injury-history-heading">
              <div>
                <Activity size={16} />
                <strong>历史记录</strong>
              </div>
              <small>全量病史，不受当前周期截断</small>
            </div>
            {visibleRecords.length ? (
              <div className="injury-timeline">
                {visibleRecords.map((record) => {
                  ...
                  return (
                    <article
                      key={record.id}
                      style={{ '--injury-color': meta.color } as CSSProperties}
                    >
                      <i />
                      <time>{formatDate(record.onsetDate)}</time>
                      <div>
                        <strong>{record.injuryName}</strong>
                        <span>
                          {sideLabels[record.side]} · {record.bodyPart} · 疼痛 {record.painScore}/10
                        </span>
                      </div>
              ...
              <div className="injury-history-empty">
            ...
            {records.length > 4 && (
              <button
                className="injury-history-more"
                type="button"
                onClick={() => setExpanded((value) => !value)}
              >
                {expanded ? '收起记录' : `查看全部${records.length}条记录`}
              </button>
            )}
          </div>
        </div>
      ...
      {message && (
        <div className={message.includes('已') ? 'injury-message success' : 'injury-message'}>
          {message}
        </div>
      )}
      ...
      {dialogOpen && (
        <div
          className="injury-dialog-overlay"
          role="presentation"
          onMouseDown={(event) => {
        ...
        >
          ...
          >
            <header>
              <div>
                <small>{isAthlete ? 'PAIN FEEDBACK' : 'HEALTH RECORD'}</small>
                <h2 id="injury-dialog-title">
                  {isAthlete ? '提交疼痛反馈' : '新增伤病与恢复记录'}
                </h2>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submit}>
              <div className="injury-form-grid">
                <label>
                  ...
                  >
                    {['肩部', '肘部', '腕部', '腰背部', '髋部', '膝部', '踝部', '颈部', '其他'].map(
                      (part) => (
                ...
                <label>
                  ...
                  <select
                    value={form.side}
                    onChange={(event) =>
                      setForm({ ...form, side: event.target.value as InjuryRecord['side'] })
                    }
                  >
                    <option value="unspecified">未区分</option>
                ...
                <label className="wide">
                  <span>{isAthlete ? '不适情况' : '问题名称或诊断'}</span>
                  <input
                    required
                    maxLength={80}
                    placeholder={isAthlete ? '例如：划桨时右肩疼痛' : '例如：右肩袖劳损'}
                    value={form.injuryName}
                    onChange={(event) => setForm({ ...form, injuryName: event.target.value })}
                  />
                </label>
                <label>
                  <span>首次出现日期</span>
                  <input
                    ...
                    onChange={(event) => setForm({ ...form, onsetDate: event.target.value })}
                  />
                </label>
                <label>
                  <span>疼痛评分（0—10）</span>
                  <input
                    ...
                    onChange={(event) =>
                      setForm({ ...form, painScore: Number(event.target.value) })
                    }
                  />
                </label>
                {!isAthlete && (
                  <label>
                    ...
                    <select
                      value={form.status}
                      onChange={(event) =>
                        setForm({ ...form, status: event.target.value as InjuryStatus })
                      }
                    >
                ...
                {!isAthlete && (
                  <label className="wide">
                    ...
                    <textarea
                      maxLength={500}
                      placeholder="例如：暂停大重量卧拉，水上训练控制在U2以下"
                ...
                {!isAthlete && (
                  <label className="wide">
                    ...
                    <textarea
                      maxLength={500}
                      placeholder="填写康复动作、频次及阶段目标"
                      value={form.rehabPlan}
                      onChange={(event) => setForm({ ...form, rehabPlan: event.target.value })}
                    />
                  </label>
                )}
                <label className="wide">
                  ...
                  <textarea
                    maxLength={800}
                    placeholder={
                      isAthlete ? '描述什么时候疼、哪些动作会加重' : '填写检查结果或其他注意事项'
                    }
                    value={form.note}
              ...
              <footer>
                <p>
                  {isAthlete
                    ? '提交后由教练确认，运动员不能修改正式伤病记录。'
                    : '保存后形成新的历史记录，不覆盖以前的数据。'}
                </p>
                <button
                  type="button"
