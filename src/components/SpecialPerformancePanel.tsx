...
export function SpecialPerformancePanel({ project, from, to }: Props) {
  ...
  return <section className="special-performance-panel">
    <header><div><span>SPECIAL RESULTS</span><h2>专项距离成绩与排名</h2><p>{project} · {from} 至 {to} · 专项成绩记录</p></div>{loading && <RefreshCw className="spin" />}</header>
    {error ? <div className="special-performance-empty error">{error}</div> : <>
      ...
      {!loading && !events.length ? <div className="special-performance-empty"><FileSpreadsheet/><strong>当前范围内暂无专项距离成绩</strong><span>可调整全局日期范围或项目后再次查看。</span></div> : <div className="special-performance-events">{events.map((event) => <article key={event.id}>
