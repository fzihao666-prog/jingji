...
export function ChampionModelBenchmark({
  benchmark,
  loading,
}: {
  benchmark: ChampionBenchmarkPayload | null;
  loading: boolean;
}) {
  ...
  if (loading)
    return <div className="champion-benchmark-empty">正在读取冠军模型标准与个人测试数据…</div>;
  if (!benchmark || !rows.length || !radarData.length)
    return <div className="champion-benchmark-empty">模型数据待配置</div>;
  ...
  const weakDimensions = [...validDimensions]
    ...
    .slice(0, 3);
  const dimensionRows = (dimension: ChampionBenchmarkDimension) =>
    rows
      .filter(
        (row) =>
          (dimensionMetricCodes[dimension.key] || []).includes(row.code) && row.score !== null
      )
      .sort((left, right) => (right.priorityIndex || 0) - (left.priorityIndex || 0))
      .slice(0, 3);

  return (
    <div className="champion-benchmark">
      <div className="champion-benchmark-body">
        <section className="champion-chart-panel">
          <div className="champion-radar-canvas">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData} outerRadius="74%">
                <PolarGrid radialLines stroke="#d7e4e6" />
                <PolarAngleAxis
                  dataKey="name"
                  tick={{ fontSize: 10, fill: '#495f66', fontWeight: 800 }}
                />
                <PolarRadiusAxis
                  angle={90}
                  domain={[0, 120]}
                  tick={{ fontSize: 8, fill: '#7c9096' }}
                  tickCount={7}
                />
                <Tooltip
                  formatter={(value, name) => [
                    `${formatNumber(Number(value), 1)}`,
                    name === 'current' ? '当前水平' : '冠军标准',
                  ]}
                  contentStyle={{
        ...
        <aside className="champion-insight-panel">
          <div>
            <Activity size={16} />
            <p>{benchmark.summary.primaryGap}</p>
          </div>
          {weakDimensions.map((dimension) => {
            ...
            return (
              <article
                key={dimension.key}
                style={{ '--champion-row': state.color } as CSSProperties}
              >
                <span>
                  <b>{label.cn}</b>
                  <em>{state.label}</em>
                ...
                <strong>
                  {dimension.current === null ? '—' : `${formatNumber(dimension.current, 1)} / 100`}
                </strong>
                <small>
                  维度差距{' '}
                  {dimension.gap === null ? '—' : formatNumber(Math.max(0, dimension.gap), 1)} ·
                  补强优先级{' '}
                  {dimension.priorityIndex === null
                    ? '—'
                    : formatNumber(dimension.priorityIndex, 1)}
                </small>
                {keyMetrics.map((row) => {
                  const range =
                    row.targetMin !== null && row.targetMax !== null
                      ? `${formatNumber(row.targetMin, 1)}-${formatNumber(row.targetMax, 1)} ${row.unit}`
                      : '未设区间';
                  return (
                    <p key={row.code}>
                      <b>{row.label}</b> 当前{' '}
                      {row.value === null
                        ? '未测试'
                        : `${formatNumber(row.value, row.unit === 's' || row.unit === '秒' ? 1 : 1)} ${row.unit}`}
                      ，冠军参考 {range}。
                    </p>
                  );
                })}
      ...
      <footer>
        <Target size={15} />
        <span>{benchmark.summary.source}</span>
      </footer>
    </div>
  );
}
