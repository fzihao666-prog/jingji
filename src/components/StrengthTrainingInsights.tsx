...
type ContentAnalysisItem = {
  name: StrengthContentAnalysisCategory;
  count: number;
  percent: number;
  completion: number;
  done: number;
...
function isWaterSession(session: StrengthTrainingSession) {
  const environments = new Set(
    session.sets.map((set) => set.trainingEnvironment || session.structureType)
  );
  return (
    environments.has('水上') ||
    environments.has('泳池') ||
    /水上|泳池|划行/.test(
      `${session.sessionLabel} ${session.trainingType} ${session.structureType}`
    )
  );
}
...
function metricCards(sessions: StrengthTrainingSession[]) {
  ...
  const intensityValues = sets
    .map((set) => set.intensityPercent)
    .filter((value): value is number => value !== null);
  return {
    rpe: rpeValues.length
      ? round(rpeValues.reduce((sum, value) => sum + value, 0) / rpeValues.length, 1)
      : 0,
    duration: round(duration),
    load: round(load),
    intensity: intensityValues.length
      ? round(intensityValues.reduce((sum, value) => sum + value, 0) / intensityValues.length)
      : 0,
...
function EmptyChart({ text }: { text: string }) {
  return (
    <div className="strength-insight-empty">
      <BarChart3 size={24} />
      <span>{text}</span>
    </div>
  );
...
function RoundRatioChart({
  ...
  return (
    <div className="strength-round-chart">
      <div
        className="strength-ratio-orb"
        style={{ background: conicGradient(items, colors) }}
      ...
      >
        <i>
          <strong>{active?.name || center}</strong>
          <small>
            {active ? `${active.percent}% · ${round(active.value).toLocaleString()} AU` : unit}
          </small>
        </i>
        {active && tooltip && (
          <div className="strength-orb-tooltip" style={{ left: tooltip.x, top: tooltip.y }}>
            <strong>{active.name}</strong>
            <span>{active.percent}%</span>
            <small>{round(active.value).toLocaleString()} AU</small>
          </div>
        )}
      </div>
      <div className="strength-round-list">
        {segments.map((item) => (
          <div
...
function OverviewCards({ sessions }: { sessions: StrengthTrainingSession[] }) {
  ...
  return (
    <section className="strength-kpi-grid">
...
function WaterLandLoadPanel({ items }: { items: LoadBreakdownItem[] }) {
  ...
  return (
    <div className="strength-load-split">
      ...
      <div
        className="load-split-track"
        aria-label={`水上负荷 ${water.percent}%，陆上负荷 ${land.percent}%`}
      >
        {items.map((item) => (
          <i key={item.name} style={{ width: `${item.percent}%`, background: item.color }} />
        ))}
      </div>
        ...
        {items.map((item) => (
          <article key={item.name}>
            ...
            <dl>
              <div>
                <dt>训练时间</dt>
                <dd>{item.duration.toLocaleString()} min</dd>
              </div>
              <div>
                <dt>训练距离</dt>
                <dd>{item.distance.toLocaleString()} km</dd>
              </div>
              <div>
                <dt>平均 RPE</dt>
                <dd>{item.rpe || '—'}</dd>
...
function LessonCompositionPanel({ items }: { items: LessonBreakdownItem[] }) {
  ...
  return (
    <div className="strength-lesson-composition">
      <div className="lesson-summary">
        <span>主课型</span>
        <strong>{dominant.name}</strong>
        <small>
          {dominant.percent}% · {dominant.value.toLocaleString()} AU · {dominant.sessions} 场
        </small>
      </div>
      <div className="lesson-rank-list">
        {visible.map((item) => (
          <article key={item.name}>
            ...
            <div className="lesson-rank-track">
              <i style={{ width: `${item.percent}%`, background: item.color }} />
            </div>
            <footer>
              <span>{item.value.toLocaleString()} AU</span>
              <span>{item.duration.toLocaleString()} min</span>
              <span>{item.sets} 项</span>
...
function ContentAnalysisPanel({
  ...
  return (
    <div className="strength-category-execution">
      <div className="category-execution-summary">
        ...
        <article>
          <span>总完成率</span>
          <strong>
            {total ? round((completed / total) * 100) : '—'}
            <small>{total ? '%' : ''}</small>
          </strong>
          <small>
            {completed} / {total} 项完成
        ...
        <article>
          ...
          <small>
            {unclassified
              ? '水上、跑步等不纳入八类分析'
              : `${leading.percent}% · ${leading.count} 项`}
          </small>
        </article>
      </div>
      <div className="category-execution-list">
        {items.map((item) => (
          <article key={item.name}>
            ...
            <footer>
              <span>构成 {item.percent}%</span>
              <span>
                完成 {item.done}/{item.count}
              </span>
            </footer>
          </article>
        ))}
...
function BodyPositionMapPanel({
  items,
}: {
  items: Array<{ name: StrengthBodyPosition; value: number }>;
}) {
  const total = items.reduce((sum, item) => sum + item.value, 0);
  if (!total) return <EmptyChart text="暂无身体位置数据" />;
  const bodyRegions: Record<
  ...
  const dominant = enriched.reduce(
    (best, item) => (item.value > best.value ? item : best),
    enriched[0]
  );
  return (
    <div className="strength-body-position-map">
      <div className="body-map-stage">
        <img src="/assets/strength-anatomy-front.png" alt="身体部位训练分布示意图" />
        <div
          className={`body-region full ${items.find((item) => item.name === '全身')?.value ? 'active' : ''}`}
        />
        {enriched.map(
          (item) =>
            item.name !== '全身' && (
              <div
                className={`body-region ${item.region}`}
                key={`${item.name}-region`}
                style={{
                  opacity: Math.max(0.18, Math.min(0.58, 0.2 + (item.percent / 100) * 0.7)),
        ...
        <svg
          className="body-map-connectors"
          viewBox="0 0 100 100"
          aria-hidden="true"
          preserveAspectRatio="none"
        >
          {enriched.map((item) => (
            <g key={`${item.name}-connector`}>
              <line
                x1={item.pointX}
                y1={item.pointY}
                x2={item.labelX}
        ...
        {enriched.map((item) => (
          <div
            className={`body-map-label ${item.align} ${item.value ? 'active' : ''}`}
            key={item.name}
            style={
              {
                left: `${item.labelX}%`,
                top: `${item.labelY}%`,
                '--hotspot-size': `${Math.max(32, Math.min(62, 32 + item.percent * 0.55))}px`,
              } as CSSProperties
            }
          >
...
export function StrengthOverviewPanel({ sessions }: { sessions: StrengthTrainingSession[] }) {
  const {
    ...
    const trend = recent.map((session) => ({
      date: session.trainingDate.slice(5),
      load: round(sessionLoad(session)),
      rpe: session.rpe || 0,
    }));
    ...
    const dailyMap = new Map<
      string,
      { date: string; label: string; distance: number; duration: number }
    >();
    ...
    const contentAnalysisMap = new Map(
      STRENGTH_CONTENT_ANALYSIS_CATEGORIES.map((name) => [name, 0])
    );
    const positionMap = new Map(STRENGTH_BODY_POSITIONS.map((name) => [name, 0]));
    const contentCompletionMap = new Map(
      STRENGTH_CONTENT_ANALYSIS_CATEGORIES.map((name) => [name, { total: 0, done: 0 }])
    );
    let unclassifiedContent = 0;
    const loadRatioMap = new Map(
      ['水上', '陆上'].map((name) => [
        name,
        { value: 0, sessions: 0, duration: 0, distance: 0, rpeSum: 0, rpeCount: 0 },
      ])
    );
    const lessonRatioMap = new Map(
      lessonTypes.map((name) => [
        name,
        {
          value: 0,
          sessions: 0,
  ...
  const dailyTickInterval =
    daily21Weeks.length > 60 ? 13 : daily21Weeks.length > 31 ? 6 : daily21Weeks.length > 14 ? 3 : 0;

  return (
    <>
      <OverviewCards sessions={sessions} />
      <section className="strength-dashboard-grid">
        <article className="strength-chart-card strength-daily-volume-card">
          <header>
            <div>
              <span>每日训练量</span>
              <h2>训练距离与训练时间</h2>
            </div>
            <small>柱状：训练距离 · 折线：训练时间</small>
          </header>
          <div className="strength-chart-area daily-volume">
            {hasDailyTraining ? (
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <CartesianGrid stroke="#e8efef" vertical={false} />
                  <XAxis
                    dataKey="label"
                  ...
                  <Tooltip
                    ...
                    formatter={(value, name) =>
                      name === '训练时间' ? [`${value} min`, name] : [`${value} km`, name]
                    }
                    contentStyle={{
                      border: '1px solid #d5e2e4',
                      borderRadius: 8,
                      boxShadow: '0 10px 24px rgba(7,59,76,.1)',
                      fontSize: 10,
                    }}
                  />
                  <Bar
              ...
              <EmptyChart text="导入训练距离或训练时间后显示日趋势" />
            )}
          </div>
        </article>
        <article className="strength-chart-card strength-load-split-card">
          <header>
            <div>
              <span>水陆训练负荷</span>
              <h2>水上与陆上负荷</h2>
            ...
            <small>
              {loadTotal
                ? `水上 ${waterPercent}% · 陆上 ${landPercent}%`
                : '按 sRPE 与训练时间汇总'}
            </small>
          </header>
          <WaterLandLoadPanel items={loadRatio} />
        </article>
        <article className="strength-chart-card strength-lesson-card">
          <header>
            <div>
              <span>训练环境与课型</span>
              <h2>训练课类型构成</h2>
            </div>
            <small>按负荷、时间、动作项和完成率汇总</small>
          </header>
          <LessonCompositionPanel items={lessonRatio} />
        </article>
        <article className="strength-chart-card">
          <header>
            <div>
              <span>最近 7 日</span>
              <h2>训练负荷趋势</h2>
            </div>
            <small>柱状：训练负荷 · 折线：RPE</small>
          </header>
          <div className="strength-chart-area">
            {trend.length ? (
              <ResponsiveContainer width="100%" height="100%">
              ...
              <EmptyChart text="导入训练记录后显示近 7 日趋势" />
            )}
          </div>
        </article>
        <article className="strength-chart-card strength-category-execution-card">
          <header>
            <div>
              <span>训练内容与执行</span>
              <h2>训练内容分析</h2>
            </div>
            <small>训练项占比 · 分类完成率 · 范围外提示</small>
          </header>
          <ContentAnalysisPanel items={contentAnalysis} unclassified={unclassifiedContent} />
        </article>
        <article className="strength-chart-card strength-body-map-card">
          <header>
            <div>
              <span>动作覆盖</span>
              <h2>身体位置训练分布</h2>
            </div>
            <small>人体部位热点 · 训练项占比</small>
          </header>
          <BodyPositionMapPanel items={positions} />
        </article>
      </section>
    </>
  );
...
export function StrengthAnalysisPanel({ sessions }: { sessions: StrengthTrainingSession[] }) {
  ...
  const data = useMemo(() => {
    const ordered = [...sessions].sort((a, b) => a.trainingDate.localeCompare(b.trainingDate));
    const limited =
      period === 'week' ? ordered.slice(-7) : period === 'month' ? ordered.slice(-30) : ordered;
    const daily = limited.map((session) => ({
      date: session.trainingDate.slice(5),
      volume: round(session.distanceKm || session.volume / 1000, 1),
      duration: round(session.durationMin || 0),
    }));
    const intensity = STRENGTH_INTENSITY_ZONES.map((zone) => ({
      name: zone,
      value: limited.reduce(
        (sum, session) =>
          sum +
          session.sets.filter((set) => (set.intensityZone || session.intensityZone) === zone)
            .length,
        0
      ),
    }));
    ...
    limited.forEach((session) =>
      session.sets.forEach((set) => {
        const environment =
          set.trainingEnvironment || (session.structureType.includes('水') ? '水上' : '陆上');
        environmentMap.set(
          environment,
          (environmentMap.get(environment) || 0) +
            (set.durationMin || session.durationMin / Math.max(1, session.sets.length) || 1)
        );
      })
    );
    const water = [...environmentMap]
      .filter(([name]) => ['水上', '泳池'].includes(name))
      .reduce((sum, [, value]) => sum + value, 0);
    const land = [...environmentMap]
      .filter(([name]) => !['水上', '泳池'].includes(name))
      .reduce((sum, [, value]) => sum + value, 0);
    const sessionTypes = [...environmentMap]
  ...
  return (
    <>
      <section className="strength-analysis-summary">
        <article>
          <span>总训练量</span>
          <strong>
            {stats.distance > 0
              ? stats.distance.toLocaleString()
              : round(stats.volume / 1000, 1).toLocaleString()}{' '}
            <small>{stats.distance > 0 ? 'km' : '千kg·reps'}</small>
          </strong>
        </article>
        <article>
          <span>训练时间</span>
          <strong>
            {stats.duration.toLocaleString()} <small>min</small>
          </strong>
        </article>
        <article>
          <span>平均 RPE</span>
        ...
        <article className="load">
          <span>累计训练负荷</span>
          <strong>
            {stats.load.toLocaleString()} <small>AU</small>
          </strong>
        </article>
      </section>
      <section className="strength-dashboard-grid analysis">
        <article className="strength-chart-card">
          <header>
            ...
            <div className="strength-period-switch">
              <button
            ...
            {data.daily.length ? (
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={data.daily}>
                  ...
                  <YAxis yAxisId="volume" tick={{ fontSize: 9 }} />
                  <YAxis yAxisId="time" orientation="right" tick={{ fontSize: 9 }} />
                  <Tooltip />
                  <Bar
                    yAxisId="volume"
                    dataKey="volume"
                    name={stats.distance > 0 ? '训练距离 km' : '训练量 千kg·reps'}
                    fill="#0d9488"
                    radius={[4, 4, 0, 0]}
                  />
                  <Line
        ...
        <article className="strength-chart-card">
          <header>
            <div>
              <span>U3 / U2 / U1 / AT / TPT / AN / ATP</span>
              <h2>训练强度分布</h2>
            </div>
          </header>
          <div className="strength-donut-layout">
            {intensityTotal ? (
              <div
                className="strength-css-donut"
                style={{ background: conicGradient(data.intensity, palette) }}
              >
                <i>
                  <strong>
                    {round(
                      (Math.max(...data.intensity.map((item) => item.value)) / intensityTotal) *
                        100,
                      1
                    )}
                    %
              ...
              <EmptyChart text="暂无强度区间数据" />
            )}
            <div className="strength-chart-list">
              {data.intensity.map((item, index) => (
                <div key={item.name}>
                  <i style={{ background: palette[index] }} />
                  <span>{item.name}</span>
                  <strong>
                    {intensityTotal ? round((item.value / intensityTotal) * 100, 1) : 0}%
                  </strong>
                </div>
              ))}
            </div>
        ...
        <article className="strength-chart-card ratio-card">
          <header>
            <div>
              <span>环境结构</span>
              <h2>水陆训练负荷占比</h2>
          ...
          <div className="water-land-ratio">
            <strong>
              {environmentTotal ? round((data.water / environmentTotal) * 100) : 0}
              <small>:</small>
              {environmentTotal ? round((data.land / environmentTotal) * 100) : 0}
            </strong>
            <div className="ratio-track">
              <i
                style={{
                  width: `${environmentTotal ? (data.water / environmentTotal) * 100 : 0}%`,
                }}
              />
            </div>
            <div>
              <span>
                水上训练{' '}
                <b>{environmentTotal ? round((data.water / environmentTotal) * 100) : 0}%</b>
              </span>
              <span>
                陆上训练{' '}
                <b>{environmentTotal ? round((data.land / environmentTotal) * 100) : 0}%</b>
              </span>
            </div>
            <small>按训练时间与动作记录汇总</small>
          </div>
        </article>
        <article className="strength-chart-card">
          <header>
          ...
          <div className="strength-chart-area compact">
            {data.sessionTypes.length ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={data.sessionTypes}
                  layout="vertical"
                  margin={{ left: 14, right: 38 }}
                >
                  <CartesianGrid stroke="#e8efef" horizontal={false} />
...
export function StrengthAssessmentPanel({ tests }: { tests: StrengthTest[] }) {
  const teamTests = useMemo(() => {
    ...
    tests.forEach((test) =>
      groups.set(test.testDate, [...(groups.get(test.testDate) || []), test])
    );
      ...
      .map(([testDate, group]) => {
        const metrics = Object.fromEntries(
          STRENGTH_METRICS.map((metric) => {
            const values = group
              .map((test) => test.metrics[metric.key])
        ...
        const targets = Object.fromEntries(
          STRENGTH_METRICS.map((metric) => {
            const values = group
              .map((test) => test.targets[metric.key])
              .filter((value): value is number => typeof value === 'number');
            return [
              metric.key,
              values.length
  ...
  const cards = assessmentMetrics.map((item) => {
    ...
    const change =
      typeof value === 'number' && typeof old === 'number' && old !== 0
        ? round(((value - old) / old) * 100, 1)
        : null;
    return { ...item, unit: definition?.unit || '', value, old, change };
  });
  const history = [...teamTests]
    .reverse()
    .map((test) => ({
      date: test.testDate.slice(5),
      squat: test.metrics.squatKg,
      bench: test.metrics.benchPressKg,
      core: test.metrics.frontPlankSec,
    }));
  const radarData = cards.map((item) => {
    const target = current?.targets[item.key];
    const score =
      typeof item.value === 'number' && typeof target === 'number' && target > 0
        ? Math.min(110, round((item.value / target) * 100))
        : typeof item.value === 'number'
          ? 80
          : 0;
    const previousScore =
      typeof item.old === 'number' && typeof target === 'number' && target > 0
        ? Math.min(110, round((item.old / target) * 100))
  ...
  if (!current)
    return (
      <section className="strength-empty results">
        <Target size={30} />
        <strong>还没有体能测试记录</strong>
        <span>录入队员体能测试后，这里会展示全队平均能力与历史变化。</span>
      </section>
    );
  return (
    <>
      <section className="strength-assessment-meta">
        <div>
          <span>最近测试</span>
          <strong>{current.testDate}</strong>
        </div>
        <div>
          <span>对比周期</span>
          <strong>
            {previous ? `${previous.testDate} → ${current.testDate}` : '暂无上次测试'}
          </strong>
        </div>
        <div>
          <span>本次覆盖</span>
          <strong>{current.athleteCount} 人</strong>
        </div>
      </section>
      <section className="strength-assessment-cards">
        {cards.map((item, index) => (
          <article key={item.key}>
            ...
            <em className={item.change !== null && item.change < 0 ? 'down' : ''}>
              {item.change === null
                ? '待形成对比'
      ...
      <section className="strength-dashboard-grid assessment">
        <article className="strength-chart-card">
          <header>
            <div>
              <span>历史测试</span>
        ...
        <article className="strength-chart-card">
          ...
          <div className="strength-radar-wrap">
            <svg
              className="strength-native-radar"
              viewBox="0 0 220 220"
              role="img"
              aria-label="本次测试与上次测试能力雷达图"
            >
              {[22, 43, 65, 86].map((radius) => (
                <polygon
                  key={radius}
                  points={radarPoints(
                    radarData.map(() => 110),
              ...
              {radarData.map((_, index) => {
                ...
                return (
                  <line
                    ...
                    x1="110"
                    y1="110"
                    x2={110 + Math.cos(angle) * 86}
                    y2={110 + Math.sin(angle) * 86}
                  />
                );
              })}
              <polygon
                points={radarPoints(radarData.map((item) => item.previous))}
                className="radar-previous"
              />
              <polygon
                points={radarPoints(radarData.map((item) => item.current))}
              ...
              {radarData.map((item, index) => {
                ...
                return (
                  <text
                    ...
                    x={110 + Math.cos(angle) * 104}
                    y={114 + Math.sin(angle) * 99}
                    textAnchor={
            ...
            <div className="strength-radar-legend">
              <span>
                <i />
                上次测试
              </span>
      ...
      <section className="strength-assessment-table">
        <header>
          <div>
            <span>评估结果</span>
            <h2>五类能力摘要</h2>
        ...
        {cards.map((item) => (
          <div className="assessment-row" key={item.key}>
            <strong>{item.domain}</strong>
            <span>{item.label}</span>
            <span>
              {item.old ?? '—'} {item.old !== undefined ? item.unit : ''}
            </span>
            <span>
              {item.value ?? '—'} {item.value !== undefined ? item.unit : ''}
            </span>
            <span className={item.change !== null && item.change < 0 ? 'down' : 'up'}>
              {item.change === null ? '—' : `${item.change >= 0 ? '+' : ''}${item.change}%`}
            </span>
            <em>{item.change === null ? '待对比' : item.change >= 0 ? '提升' : '关注'}</em>
          </div>
        ))}
      </section>
    </>
...
export function StrengthPlanCategoryTabs({
  value,
  onChange,
}: {
  value: StrengthTrainingCategory;
  onChange: (category: StrengthTrainingCategory) => void;
}) {
  return (
    <nav className="strength-category-tabs" aria-label="体能训练类型">
      {STRENGTH_TRAINING_CATEGORIES.map((category) => (
        <button
