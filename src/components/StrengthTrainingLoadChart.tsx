    ...
    <section className="strength-load-visual" aria-label="训练负荷趋势">
      <header>
        <div>
          ...
          <h3>训练负荷趋势</h3>
          <p>柱形表示每场训练量，折线表示平均RPE；同日多场分别记录。</p>
        </div>
        <div className="strength-load-legend" aria-hidden="true">
          <span>
            <i />
            训练量
          </span>
          ...
          <ComposedChart data={data} margin={{ top: 10, right: 8, left: -10, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(value, name) =>
                name === '平均RPE'
                  ? [value ?? '未记录', name]
                  : [`${Number(value).toLocaleString()} kg·reps`, name]
              }
              contentStyle={{
