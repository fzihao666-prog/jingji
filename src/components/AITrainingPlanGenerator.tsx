...
function relativeExerciseLoad(
  exercise: AIPlan['weeklyPlans'][number]['days'][number]['exercises'][number]
) {
...
function AITrainingPlanGeneratorContent({ athlete, onSaved }: Pick<Props, 'athlete' | 'onSaved'>) {
  ...
  const dashboardData = useMemo(() => {
    ...
    const heatmapWeeks: Array<{
      week: string;
      focus: string;
      days: Array<{ day: string; load: number; prescriptions: number }>;
    }> = [];
    ...
    return {
      ...
      metrics: {
        ...
        averageIntensity: percentages.length
      ...
      <div className="ai-training-container preview">
        <div className="preview-header">
          <div className="plan-title-section">
            ...
            <div className="plan-meta">
              <span>
                <Calendar size={14} /> {generatedPlan.durationWeeks} 周训练
              </span>
              <span>
                <Clock size={14} /> {generatedPlan.scheduleLabel}
              </span>
              <span>
                <Target size={14} /> {generatedPlan.progression.phase}
              </span>
            </div>
          </div>
          <button
        ...
        <div className="preview-content">
          ...
          <div className="charts-section">
            <div className="charts-section-heading">
              <div>
                <BarChart3 size={19} />
                <span>
                  <strong>体能训练数据看板</strong>
                  <small>保存前检查训练量、强度与结构是否均衡</small>
                </span>
              </div>
            ...
            <div className="plan-dashboard-strip">
              <div>
                <span>训练周期</span>
                <strong>{generatedPlan.durationWeeks}</strong>
                <small>WEEKS</small>
              </div>
              <div>
                <span>训练日</span>
                <strong>{dashboardData.metrics.days}</strong>
                <small>SESSIONS</small>
              </div>
              <div>
                <span>处方条目</span>
                <strong>{dashboardData.metrics.prescriptions}</strong>
                <small>ITEMS</small>
              </div>
              <div>
                <span>平均强度</span>
                <strong>{dashboardData.metrics.averageIntensity || '—'}</strong>
                <small>{dashboardData.metrics.averageIntensity ? '% 1RM' : '未标注'}</small>
              </div>
              <div className="peak">
                <span>峰值周</span>
                <strong>{dashboardData.metrics.peakWeek}</strong>
            ...
            <div className="charts-grid enhanced">
              <div className="chart-card wide chart-primary">
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={loadChartData}>
                    ...
                    <XAxis
                      ...
                      tick={{ fontSize: 11 }}
                      tickLine={false}
                      axisLine={{ stroke: '#b9cccf' }}
                    />
                    <YAxis domain={[0, 100]} tickLine={false} axisLine={false} />
                    <Tooltip
                      formatter={(value, name) => [value, name === 'load' ? '负荷指数' : name]}
                    />
                    <Area
                      type="monotone"
                      dataKey="load"
                ...
                <div className="chart-inline-stats">
                  {loadChartData.map((item) => (
                    <span key={item.week}>
                      <b>{item.week}</b>
                      {item.days}天 · {item.prescriptions}条
                    </span>
                  ))}
                </div>
              </div>
                ...
                <div className="chart-legend-list">
                  {exerciseDistribution.map((entry) => (
                    <span key={entry.key}>
                      ...
                      <em>
                        {distributionTotal
                          ? Math.round((entry.value / distributionTotal) * 100)
                          : 0}
                        %
                      </em>
                    </span>
                  ...
                  <BarChart data={dashboardData.intensityByWeek}>
                    ...
                    <Tooltip
                      formatter={(value, name) => [
                        `${value} 条`,
                        name === 'low' ? '低强度' : name === 'medium' ? '中强度' : '高强度',
                      ]}
                    />
                    <Legend
                      formatter={(value) =>
                        value === 'low' ? '低强度' : value === 'medium' ? '中强度' : '高强度'
                      }
                    />
              ...
              <div className="chart-card">
                ...
                {dashboardData.exerciseVolumes.length > 0 ? (
                  <ResponsiveContainer
                    width="100%"
                    height={Math.max(230, dashboardData.exerciseVolumes.length * 34)}
                  >
                    <BarChart
                      data={dashboardData.exerciseVolumes}
                      layout="vertical"
                  ...
                  <div className="chart-empty">AI未返回可计算的项目处方</div>
                )}
              </div>
              ...
              <div className="chart-card">
                ...
                {dashboardData.dailyLoads.length > 0 ? (
                  <ResponsiveContainer
                    width="100%"
                    height={Math.max(230, dashboardData.dailyLoads.length * 31)}
                  >
                    <BarChart
                      data={dashboardData.dailyLoads}
                      layout="vertical"
                    ...
                    >
                      ...
                      />
                      <Tooltip
                        formatter={(value, name) => [value, name === 'load' ? '相对负荷' : name]}
                      />
                      <Bar dataKey="load" fill="#1b9d95" radius={[0, 5, 5, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="chart-empty">AI未返回训练日处方</div>
                )}
              </div>
              ...
              <div className="chart-card wide heatmap-card">
                <div className="chart-card-heading">
                  ...
                  <div>
                    <h4>周期训练密度热力图</h4>
                    <p>颜色越深，代表该训练日的相对处方负荷越高</p>
                  </div>
                </div>
                {hasHeatmapDays ? (
                  <div className="microcycle-heatmap" role="img" aria-label="周期训练密度热力图">
                          ...
                          {week.days.map((day, index) => {
                            ...
                            return (
                              <div
                                ...
                                key={`${day.day}-${index}`}
                                style={{
                                  backgroundColor: `rgba(11, 89, 99, ${0.1 + ratio * 0.84})`,
                                }}
                                title={`${day.day}：相对负荷 ${day.load}，${day.prescriptions} 条处方`}
                              >
                                <span>{day.day}</span>
                                <strong>{day.load}</strong>
                                <small>{day.prescriptions} 条</small>
                              </div>
                            );
                          })}
                        </div>
                  ...
                  <div className="chart-empty">AI未返回可展示的周期训练日</div>
                )}
                {hasHeatmapDays && (
          ...
          <div className="preview-actions">
            ...
            <button className="btn-primary save-plan" onClick={handleSave} disabled={saving}>
              {saving ? (
                <>
