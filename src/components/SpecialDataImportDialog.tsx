...
export function SpecialDataImportDialog({ project, onClose, onCommitted }: Props) {
  ...
  return (
    <div
      className="special-data-dialog-backdrop"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <section
        className="special-data-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="special-data-import-title"
      >
        <header>
          <div>
          ...
          <button
            type="button"
            className="special-data-icon-button"
            onClick={onClose}
        ...
        {!preview ? (
          <div className="special-data-import-start">
            ...
            <div className="special-data-guidance">
              ...
              <span>
                训练日期、项目、距离、艇型、性别组别、运动员或组合、训练时段、环境条件、历史最好与三轮计时成绩。
              </span>
            </div>

            {message && (
              <div
                className={`special-data-message ${message.includes('开始下载') ? 'success' : 'error'}`}
              >
            ...
            <footer>
              <button
                type="button"
                className="special-data-button ghost"
              ...
              <button
                type="button"
                className="special-data-button primary"
          ...
          <div className="special-data-import-review">
            <div className="special-data-summary">
              ...
              <div>
                <span>需要修正</span>
                <strong className={summary.invalid ? 'danger' : ''}>{summary.invalid}</strong>
              </div>
              <div>
                <span>校验提醒</span>
                ...
                <tbody>
                  {preview.rows.map((row) => (
                    <tr key={row.rowNumber} className={row.errors.length ? 'invalid' : ''}>
                      <td>
                        {row.errors.length ? (
                          <XCircle className="row-error" size={17} />
                        ) : (
                          <CheckCircle2 className="row-ok" size={17} />
                        )}
                      </td>
                      <td>{row.rowNumber}</td>
                      <td>{row.testDate || '—'}</td>
                      ...
                      <td>
                        <strong>{row.boatClass}</strong>
                        <small>{row.genderGroup}</small>
                      </td>
                      <td>
                        <strong>{row.crewName || '未填写'}</strong>
                        <small>{row.memberNames.join('、') || '未匹配运动员'}</small>
                      </td>
                      <td>
                        {row.attemptsMs.length ? row.attemptsMs.map(formatTime).join(' / ') : '—'}
                      </td>
                      <td>
                        <strong>{formatTime(row.bestMs)}</strong>
                      </td>
                      <td>
                        <span className={row.errors.length ? 'validation-error' : 'validation-ok'}>
                          {row.errors.join('；') || row.warnings.join('；') || '校验通过'}
                        </span>
                      </td>
                    </tr>
                  ))}
            ...
            {message && (
              <div
                className={`special-data-message ${message.includes('已保存') ? 'success' : 'error'}`}
              >
            ...
            <footer>
              ...
              <button
                type="button"
                className="special-data-button primary"
                disabled={preview.valid === 0 || Boolean(busy)}
                onClick={commit}
              >
                {busy === 'commit' ? (
                  <LoaderCircle className="spin" size={17} />
