...
专项首页通过 `/api/special-training/overview` 读取专项训练统计，不读取专项成绩，服务端先复用运动员访问范围校验，再按项目、队伍与日期裁剪；首页不支持运动员筛选，运动员账号仍受本人访问权限约束。训练统计复用 `overview-service.ts` 的场次查询与团队课次去重，纯聚合规则位于 `shared/special-training.ts`；只读取真实专项课次的既有 SRPE 和原始强度，不生成演示数据。

专项首页图表使用已有 ECharts 6 依赖，按路由懒加载；`EChart.tsx` 集中注册所需图表、复用 CSS 色彩变量、监听容器尺寸并在卸载时释放实例。页面复用公共容器、筛选、卡片和空状态，不改变冠军模型实现。
...
冠军模型是项目级参考配置，不属于任何单个运动员，也不从训练记录、个人测试或体能冠军模型推导。每条已确认成绩保留 `competition_date`；当前无已确认配置时，接口返回空事件列表，页面显示“模型数据待配置”。赛艇世界最好成绩覆盖当前奥运/重点训练船型14项；赛艇陆上赛艇冠军模型独立读取 `ergometer_champion_models`，按男女各自体重范围生成每1kg连续档位，2000m与5000m保存完成时间，30分钟（20桨）保存/500m平均配速。
...
4. **新训练数据只写新模型**：不得新增对 `training_records` 的依赖；训练课次的时长、距离均以 `training_sessions` 为唯一事实源，并通过填报标记区分缺失与真实 0；
