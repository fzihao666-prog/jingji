...
专项首页通过 `/api/special-training/overview` 在一次请求中读取训练统计与专项成绩，服务端先复用运动员访问范围校验，再按项目、队伍、运动员与日期裁剪。训练统计复用 `overview-service.ts` 的场次查询与团队课次去重，纯聚合规则位于 `shared/special-training.ts`；只读取真实专项课次的既有 SRPE 和原始强度，不生成演示数据。专项成绩复用现有事件—结果读取和权限校验，兼容读取旧中文项目名并在响应中统一为项目代码，按距离、艇型、性别分别展示，避免跨规格比较。
