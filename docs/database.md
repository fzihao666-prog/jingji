...
| `coach_athletes`             | 教练—运动员多对多关系              | 仅作主管教练展示与上级关系兼容；运动员可见性由区域/项目/队伍权限决定。 |
...
- 多维授权表和审计表使服务端可以实施角色与资源范围交集，而非只依赖角色；教练对运动员的可见性由区域/项目/队伍权限决定。
...
当前已有训练、测试和导入索引，但权限计算和列表查询常按项目、队伍、区域、启用状态过滤。数据量增长后，应通过实际 `EXPLAIN QUERY PLAN` 和压测验证，再按查询补充索引，例如 `athletes(project, team, active)`、`users(athlete_id, active)`。不建议在没有查询证据时盲目增加所有组合索引，这会拖慢导入和写入。
