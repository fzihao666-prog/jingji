...
if (formal.status !== 201 || formal.payload.record.recordType !== 'formal')
  throw new Error(`正式记录创建失败：${JSON.stringify(formal)}`);

const feedback = await request(`/api/athletes/${athleteId}/injuries`, {
  method: 'POST',
  headers: athleteHeaders,
  body: JSON.stringify({
    injuryName: '划桨时右肩仍有酸痛',
    bodyPart: '肩部',
    side: 'right',
    status: 'healthy',
    painScore: 3,
    onsetDate: '2026-07-31',
    restrictions: '运动员无权设置',
    rehabPlan: '运动员无权设置',
    reviewDate: '',
    note: '热身后减轻',
  }),
...
if (
  feedback.status !== 201 ||
  feedback.payload.record.recordType !== 'feedback' ||
  feedback.payload.record.status !== 'observation' ||
  feedback.payload.record.restrictions
) {
...
const forbidden = await request(`/api/athletes/${athleteId + 999}/injuries`, {
  headers: athleteHeaders,
});
if (list.status !== 200 || list.payload.records.length !== 2 || forbidden.status !== 403)
  throw new Error('伤病记录查询或隔离验证失败');
