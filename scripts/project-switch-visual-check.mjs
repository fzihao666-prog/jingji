...
if (!rowingOptions.length || !canoeOptions.length)
  throw new Error('project athlete selectors are empty');
if (rowingSearchResults.length !== 3 || canoeSearchResults.length !== 3)
  throw new Error('athlete search did not filter by region');
const sameNamedOptions = rowingOptions.filter(
  (name) => name && !name.startsWith('全部') && canoeOptions.includes(name)
);
if (sameNamedOptions.length)
  throw new Error(`athlete options leaked between projects: ${sameNamedOptions.join(',')}`);
