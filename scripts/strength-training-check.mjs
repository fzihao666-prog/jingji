...
try {
  ...
  if (firstPreview.total !== 2 || firstPreview.valid !== 2 || firstPreview.invalid !== 0)
    throw new Error(`首次预览错误：${JSON.stringify(firstPreview)}`);
  const firstCommit = await jsonRequest(
  ...
  if (firstCommit.imported !== 2 || firstCommit.sessions !== 1)
    throw new Error(`首次提交错误：${JSON.stringify(firstCommit)}`);

  const secondPreview = await previewWorkbook(token, athlete);
  if (secondPreview.duplicate !== 2)
    throw new Error(`重复检测错误：${JSON.stringify(secondPreview)}`);
  const secondCommit = await jsonRequest(
    '/api/strength-training/import/commit',
    {
      method: 'POST',
      body: JSON.stringify({
  ...
  if (secondCommit.imported !== 2 || secondCommit.sessions !== 1)
    throw new Error(`另存场次错误：${JSON.stringify(secondCommit)}`);

  const results = await jsonRequest(
    `/api/strength-training/results?athleteId=${athlete.id}`,
    {},
    token
  );
  const importedSessions = results.sessions.filter(
    (session) => session.trainingDate === '2026-08-24' && session.sessionLabel === '自动化验证场次'
  );
  if (
    importedSessions.length !== 2 ||
    importedSessions.some((session) => session.sets.length !== 2)
  )
    throw new Error(`结果持久化错误：${JSON.stringify(importedSessions)}`);
  if (new Set(importedSessions.map((session) => session.sessionOrder)).size !== 2)
    throw new Error('同日多场训练的场次序号发生覆盖。');
  if (
    importedSessions.some(
      (session) =>
        session.durationMin !== 12 ||
        session.srpe <= 0 ||
        session.sets.some(
          (set) =>
            set.trainingCategory !== '基础力量' ||
            set.bodyPosition !== '上肢' ||
            set.trainingEnvironment !== '场馆' ||
            set.plannedWeightKg !== 57.5 ||
            set.intensityPercent !== 82 ||
            set.intensityZone !== 'AN'
        )
    )
  ) {
    ...
    JSON.stringify(
      {
        ...
        sameDayMultipleSessions: 'passed',
        sessions: importedSessions.length,
        rows: importedSessions.reduce((sum, session) => sum + session.sets.length, 0),
      },
      null,
      2
    )
