...
assert.deepEqual(summarizeDailyRpe([]), {
  averageRpe: null,
  stdRpe: null,
  lowerRpe: null,
  upperRpe: null,
...
assert.deepEqual(summarizeDailyRpe([row(1, 6)]), {
  averageRpe: 6,
  stdRpe: 0,
  lowerRpe: 6,
  upperRpe: 6,
...
assert.deepEqual(summarizeDailyRpe([row(1, 4), row(1, 8), row(2, 10)]), {
  averageRpe: 8,
  stdRpe: 2,
  lowerRpe: 6,
  upperRpe: 10,
...
assert.ok(
  Math.abs(precise.stdRpe! - Math.sqrt(14 / 3)) < 1e-12,
  '采用总体标准差，统计过程不提前舍入'
);
