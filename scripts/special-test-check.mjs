...
try {
  ...
  const specialFile = new File([templateBuffer], '竞迹专项训练导入模板.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  ...
  console.log(JSON.stringify({ specialEvents: events.payload.events.length, rankedResults: events.payload.events[0].results.length, status: 'ok' }, null, 2));
} finally {
  server.kill();
