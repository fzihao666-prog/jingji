...
const sessions = sessionRows.map((row, index) => ({
  ...
  trainingType: '力量训练',
  structureType: row[4],
  intensityZone: index === 4 ? 'TPT' : index === 3 ? 'U2' : 'AN',
  rpe: row[7],
  volume: row[6],
  durationMin: row[8],
  distanceKm: row[9],
  srpe: Number(row[7]) * Number(row[8]),
  source: 'file_import',
  sourceFilename: '训练结果.xlsx',
  modelUsed: '',
  importedAt: `${row[0]}T10:00:00Z`,
  sets: [
    {
      id: 9201 + index,
      exerciseName: row[5],
      setIndex: 1,
      ...setDefaults,
      trainingCategory: row[2],
      bodyPosition: row[3],
      trainingEnvironment: row[4],
      durationMin: row[8],
      distanceKm: row[9],
      intensityZone: index === 4 ? 'TPT' : index === 3 ? 'U2' : 'AN',
      rpe: row[7],
...
await page.route('**/api/strength-training/results?*', (route) =>
  route.fulfill({
    status: 200,
...
await page.route('**/api/strength-tests?*', (route) =>
  route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({
...
if ((await page.locator('.strength-kpi-grid article').count()) !== 5)
  throw new Error('体能总览核心指标卡数量不是5。');
if ((await page.locator('.strength-dashboard-grid .strength-chart-card').count()) !== 6)
  throw new Error('体能总览图表数量不是6。');
if (await page.getByText('低中高强度占比').count())
  throw new Error('体能总览仍显示低中高强度占比。');
if (await page.getByRole('heading', { name: '训练完成情况', exact: true }).count())
  throw new Error('体能总览仍单独显示训练完成情况。');
if ((await page.locator('.strength-load-split-card').count()) !== 1)
  throw new Error('体能总览缺少新版水陆负荷面板。');
if ((await page.locator('.strength-lesson-card').count()) !== 1)
  throw new Error('体能总览缺少新版训练课类型构成面板。');
if ((await page.getByRole('heading', { name: '训练内容分析', exact: true }).count()) !== 1)
  throw new Error('体能总览缺少训练内容分析面板。');
for (const label of [
  '交叉训练',
  '功能训练',
  '拉伸再生',
  ...
  if ((await page.getByText(label, { exact: true }).count()) < 1)
    throw new Error(`训练内容分析缺少${label}。`);
}
if ((await page.locator('.strength-body-map-card .body-map-stage img').count()) !== 1)
  throw new Error('体能总览缺少身体部位图。');
...
if ((await page.locator('.strength-category-tabs button').count()) !== 5)
  throw new Error('训练安排缺少五类训练Tab。');
...
if (firstMax !== '65' || firstWeight !== '45.5 kg')
  throw new Error(`计划重量联动错误：${firstMax}/${firstWeight}`);
...
if ((await page.locator('.strength-session-card').count()) < 1)
  throw new Error('训练记录未显示已导入场次。');
if ((await page.getByRole('button', { name: '导入训练结果', exact: true }).count()) !== 0)
  throw new Error('训练记录仍显示导入入口。');
await page.screenshot({ path: path.join(outputDirectory, '03训练记录.png'), fullPage: true });

await openStrength('训练分析');
if ((await page.locator('.strength-analysis-summary article').count()) !== 4)
  throw new Error('训练分析摘要数量不是4。');
if ((await page.locator('.strength-dashboard-grid.analysis .strength-chart-card').count()) !== 4)
  throw new Error('训练分析四宫格不完整。');
await page.screenshot({ path: path.join(outputDirectory, '04训练分析.png'), fullPage: true });

await openStrength('体能评估');
if ((await page.locator('.strength-assessment-cards article').count()) !== 5)
  throw new Error('体能评估五类能力卡不完整。');
