...
try {
  for (let attempt = 0; attempt < 100; attempt++) {
    try {
      token = (await request('/api/auth/login', { username: 'admin01', password: 'demo123' }))
        .token;
      break;
    } catch {}
  ...
  await request('/api/special-training/sessions', {
    sessions: [1, 2, 4].map((day) => ({
  ...
  const response = await request(
    `/api/special-training/overview?from=2026-09-01&to=2026-09-05&project=ROWING`
  );
  ...
  page.on('console', (message) => {
    if (
      message.type() === 'error' ||
      (/ECharts/.test(message.text()) && message.type() === 'warning')
    )
  ...
  for (const name of [
    ...
    assert.ok(headings.includes(name));
  ...
  assert.equal(
    await page.locator('.special-champion-model').innerHTML(),
    champion,
    '冠军模型发生非预期变化'
  );
  ...
  assert.ok(
    (await page.locator('.training-dashboard-metric').first().boundingBox()).height < 180,
    '移动端紧凑卡片被通用面板高度撑开'
  );
  for (const chart of await page.locator('.app-echart').all())
    assert.ok((await chart.boundingBox()).width > 0);
  ...
  console.log(
    '专项首页浏览器回归通过：真实接口、无运动员筛选、无表现趋势、日期联动、四张图表、冠军模型不变、项目切换、移动端布局。'
  );
} finally {
  await browser?.close();
  server.kill();
