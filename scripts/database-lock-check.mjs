...
async function waitForServer(target) {
  for (let attempt = 0; attempt < 160; attempt += 1) {
    if (target.child.exitCode !== null)
      throw new Error(`端口 ${target.port} 的进程提前退出：\n${target.output()}`);
    try {
...
try {
  try {
    ...
    if (processes.some((target) => /database is locked/i.test(target.output())))
      throw new Error('并发初始化仍出现数据库锁错误');
  } finally {
  ...
  try {
    ...
    if (!response.ok || !payload.records?.length)
      throw new Error(
        `并发初始化后的数据库校验失败：${response.status} ${JSON.stringify(payload)}`
      );
    console.log(
      JSON.stringify(
        {
          concurrentInitializers: processes.length,
          verifiedRecords: payload.records.length,
          databaseLock: 'resolved',
        },
        null,
        2
      )
