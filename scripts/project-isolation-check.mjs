...
try {
  ...
  const rowingRecords = await request(
    '/api/records?from=2026-07-01&to=2026-07-31&project=赛艇',
    {},
    token
  );
  const canoeRecords = await request(
    '/api/records?from=2026-07-01&to=2026-07-31&project=皮划艇',
    {},
    token
  );
  assert(missingProject.status === 400, 'records endpoint accepted a request without project');
  assert(
    rowingRecords.payload.records.every((row) => row.project === '赛艇'),
    'rowing records leaked canoe data'
  );
  assert(
    canoeRecords.payload.records.every((row) => row.project === '皮划艇'),
    'canoe records leaked rowing data'
  );
  ...
  wrongLane.append(
    'file',
    new Blob([await specialWorkbook('赛艇', rowingAthlete.name)]),
    'wrong-lane.xlsx'
  );
  const wrongPreview = await request(
    '/api/special-tests/import/preview',
    { method: 'POST', body: wrongLane },
    token
  );
  assert(
    wrongPreview.status === 200 && wrongPreview.payload.invalid === 1,
    'cross-project special test import was not rejected'
  );

  for (const [project, athlete] of [
    ['赛艇', rowingAthlete],
    ['皮划艇', canoeAthlete],
  ]) {
    const preview = await previewSpecial(project, athlete.name, token);
    assert(
      preview.status === 200 && preview.payload.valid === 1,
      `${project} special preview failed`
    );
    const commit = await request(
      '/api/special-tests/import/commit',
      { method: 'POST', body: JSON.stringify({ importId: preview.payload.importId }) },
      token
    );
    assert(
      commit.status === 200 && commit.payload.events === 1,
      `${project} special commit failed`
    );
  }
  const rowingEvents = await request(
    '/api/special-tests?from=2026-07-01&to=2026-07-31&project=赛艇',
    {},
    token
  );
  const canoeEvents = await request(
    '/api/special-tests?from=2026-07-01&to=2026-07-31&project=皮划艇',
    {},
    token
  );
  assert(
    rowingEvents.payload.events.length === 1 && rowingEvents.payload.events[0].project === '赛艇',
    'rowing special events leaked'
  );
  assert(
    canoeEvents.payload.events.length === 1 && canoeEvents.payload.events[0].project === '皮划艇',
    'canoe special events leaked'
  );
  ...
  console.log(
    JSON.stringify(
      {
        records: {
          rowing: rowingRecords.payload.records.length,
          canoe: canoeRecords.payload.records.length,
        },
        models: [rowingModel.payload.standard.version, canoeModel.payload.standard.version],
        specialEvents: {
          rowing: rowingEvents.payload.events.length,
          canoe: canoeEvents.payload.events.length,
        },
