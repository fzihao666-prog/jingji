...
try {
  ...
  const initial = await request(
    '/api/records?from=2020-01-01&to=2100-12-31&project=ROWING',
    {},
    token
  );
  assert(initial.status === 200 && initial.payload.records.length > 0, '专项训练基础记录为空');
  assert(
    initial.payload.records.some(
      (item) => item.averageHeartRate && item.averagePowerW && item.strokeRateSpm
    ),
    '数据库专项指标不完整'
  );
  ...
  const invalidDate = await request(
    ...
    { method: 'POST', body: JSON.stringify({ sessions: [{ ...baseRow, date: '2026-99-99' }] }) },
    token
  );
  assert(invalidDate.status === 400, '无效自然日期未被拒绝');
  const invalidHeartRate = await request(
    '/api/special-training/sessions',
    { method: 'POST', body: JSON.stringify({ sessions: [{ ...baseRow, maxHeartRate: 120 }] }) },
    token
  );
  ...
  const rollback = await request(
    '/api/special-training/sessions',
    {
      ...
      body: JSON.stringify({
        sessions: [
          { ...baseRow, content: rollbackContent },
          { ...baseRow, rpe: 12 },
        ],
      }),
    },
  ...
  const afterRollback = await request(
    `/api/records?from=2026-08-27&to=2026-08-27&athleteId=${athlete.id}&project=ROWING`,
    {},
    token
  );
  assert(
    !afterRollback.payload.records.some((item) => item.content === rollbackContent),
    '批量录入失败后没有完整回滚'
  );

  const saved = await request(
    '/api/special-training/sessions',
    { method: 'POST', body: JSON.stringify({ sessions: [baseRow] }) },
    token
  );
  ...
  assert(
    record?.averageHeartRate === 147 &&
      record?.maxHeartRate === 182 &&
      record?.averagePowerW === 366 &&
      record?.strokeRateSpm === 31,
    '专项指标没有按原值持久化'
  );
  ...
  assert(
    dashboard.status === 200 && dashboard.payload.training.summary.sessionCount === 1,
    `专项首页未排除演示课次：${dashboard.status} ${JSON.stringify(dashboard.payload.training?.summary || dashboard.payload.message)}`
  );
  assert(
    dashboard.payload.training.summary.durationMin === 88 &&
      dashboard.payload.training.summary.distanceKm === 17.4,
    '专项首页训练时长/距离与真实记录不一致'
  );
  assert(dashboard.payload.training.summary.load === 528, '专项首页没有沿用已有SRPE');
  assert(
    dashboard.payload.training.content.reduce((sum, row) => sum + row.count, 0) === 1,
    '专项内容课次统计错误'
  );
  const teammate = athletesResult.payload.athletes.find(
    (item) =>
      item.id !== athlete.id && item.project === athlete.project && item.team === athlete.team
  );
  assert(teammate, '缺少同队课次去重回归人员');
  // 独立测试库显式建立同队关系，避免依赖旧初始化数据的项目名称映射。
  const fixtureDb = new DatabaseSync(databasePath);
  const createdTeam = fixtureDb
    .prepare('INSERT INTO project_teams (project, name) VALUES (?, ?)')
    .run(athlete.project, '专项回归队伍');
  const team = { id: Number(createdTeam.lastInsertRowid) };
  fixtureDb
    .prepare('UPDATE athletes SET team_id = ? WHERE id IN (?, ?)')
    .run(team.id, athlete.id, teammate.id);
  fixtureDb.close();
  const teammateSaved = await request(
    '/api/special-training/sessions',
    {
      method: 'POST',
      body: JSON.stringify({ sessions: [{ ...baseRow, athleteId: teammate.id }] }),
    },
    token
  );
  ...
  assert(
    teamDashboard.status === 200 && teamDashboard.payload.training.summary.sessionCount === 1,
    '团队共同课次被重复统计'
  );
  assert(
    teamDashboard.payload.training.summary.durationMin === 88 &&
      teamDashboard.payload.training.summary.load === 528,
    '团队时长或既有负荷去重错误'
  );
  const missing = await request(
    `/api/special-training/overview?from=2099-01-01&to=2099-01-02&project=${encodeURIComponent(athlete.project)}`,
    {},
    token
  );
  assert(
    missing.payload.training.summary.durationMin === null &&
      missing.payload.training.days.length === 0,
    '空周期生成了模拟数据'
  );
  const wrongProject = await request(
    `/api/special-training/overview?from=2026-08-27&to=2026-08-27&project=${encodeURIComponent('CANOE_SLALOM')}&teamId=${team.id}`,
    {},
    token
  );
  ...
  const badRange = await request(
    `/api/special-training/overview?from=2026-09-01&to=2026-08-01&project=${encodeURIComponent(athlete.project)}`,
    {},
    token
  );
  ...
  console.log(
    JSON.stringify(
      {
        seededRecords: initial.payload.records.length,
        persistedSessionId: record.id,
        validation: 'ok',
