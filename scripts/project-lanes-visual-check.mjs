...
try {
  ...
  if (labels.join('|') !== '赛艇|皮划艇|激流')
    throw new Error(`项目标签不正确：${labels.join('|')}`);
  ...
  for (const label of labels) {
    ...
    if (selectedProject !== label)
      throw new Error(`工具栏项目未同步：预期${label}，实际${selectedProject}`);
    const filterOverflow = await projectFilter.evaluate(
      (item) => item.scrollWidth > item.clientWidth || item.scrollHeight > item.clientHeight
    );
    ...
    await page
      .locator('.athlete-picker-menu')
      .screenshot({ path: `${outputDirectory}/运动员下拉-${label}.png` });
    await page.locator('.athlete-picker-trigger').click();
  }

  await page.setViewportSize({ width: 390, height: 844 });
  await page.waitForTimeout(300);
  const mobileFilterOverflow = await projectFilter.evaluate(
    (item) => item.scrollWidth > item.clientWidth || item.scrollHeight > item.clientHeight
  );
  ...
  console.log(
    JSON.stringify({ labels, metrics, screenshots: labels.length * 3 + 1, errors }, null, 2)
  );
} finally {
  await browser.close();
}
