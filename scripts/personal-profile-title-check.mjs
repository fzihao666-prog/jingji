...
async function openPersonalArchive(page) {
  ...
  if (await picker.count()) {
    ...
    const athleteOption = page
      .locator('.athlete-picker-options > button')
      .filter({ hasText: '林舟' })
      .first();
  ...
  for (const heading of ['制胜要素分析', '训练情况', '生理生化与恢复状态', '个人 vs 团队对比']) {
    await page.getByRole('heading', { name: heading, exact: true }).waitFor();
  }
  await page.getByText('生理生化数据暂未接入', { exact: true }).waitFor();
}
...
try {
  ...
  coachPage.on('console', (message) => {
    if (message.type() === 'error') coachErrors.push(`console: ${message.text()}`);
  });
  ...
  const moduleTitle = (
    await coachPage.locator('.strength-module-title strong').textContent()
  )?.trim();
  ...
  const importButtons = await coachPage
    .getByRole('button', { name: /导入数据|下载导入模板/ })
    .count();
  const newButtons = await coachPage
    .getByRole('button', { name: '录入新测试', exact: true })
    .count();
  const editButtons = await coachPage
    .getByRole('button', { name: '编辑本次数据', exact: true })
    .count();
  if (moduleTitle !== '体能测试档案' || !posterTitle?.includes('体能测试档案'))
    throw new Error(`体能测试档案标题错误：${moduleTitle} / ${posterTitle}`);
  if (archiveRows !== 8 || radarCount !== 1 || legacyBodyMap !== 0)
    throw new Error(
      `档案结构错误：rows=${archiveRows}, radar=${radarCount}, legacy=${legacyBodyMap}`
    );
  if (importButtons !== 0 || newButtons !== 1 || editButtons !== 1)
    throw new Error(
      `教练操作入口错误：import=${importButtons}, new=${newButtons}, edit=${editButtons}`
    );

  await coachPage
    .locator('.strength-module')
    .screenshot({ path: `${outputDirectory}/体能测试档案.png` });
  const pdfDownloadPromise = coachPage.waitForEvent('download', { timeout: 60000 });
  await coachPage.getByRole('button', { name: '导出体能测试档案', exact: true }).click();
  const pdfDownload = await pdfDownloadPromise;
  await pdfDownload.saveAs(`${outputDirectory}/体能测试档案.pdf`);
  ...
  if (entryRows < 15 || livePreview !== 1)
    throw new Error(`录入工作区结构错误：rows=${entryRows}, preview=${livePreview}`);
  ...
  const previewScore = (
    await coachPage.locator('.archive-preview-stats > div').nth(1).textContent()
  )?.trim();
  if (!previewScore || previewScore.includes('—'))
    throw new Error(`实时评分未更新：${previewScore}`);
  await coachPage.screenshot({
    path: `${outputDirectory}/体能测试档案数据录入.png`,
    fullPage: true,
  });
  ...
  const editDateDisabled = await coachPage
    .locator('.archive-entry-meta input[type="date"]')
    .isDisabled();
  ...
  const mobileOverflow = await coachPage
    .locator('.archive-sheet-scroll')
    .evaluate((element) => element.scrollWidth > element.clientWidth);
  if (!mobileOverflow) throw new Error('移动端运动员表现未启用横向查看。');
  await coachPage
    .locator('.archive-sheet-scroll')
    .screenshot({ path: `${outputDirectory}/体能测试档案移动端.png` });
  ...
  athletePage.on('console', (message) => {
    if (message.type() === 'error') athleteErrors.push(`console: ${message.text()}`);
  });
  await login(athletePage, 'athlete01');
  await openPersonalArchive(athletePage);
  const athleteWriteButtons = await athletePage
    .getByRole('button', { name: /录入|编辑|保存草稿|沿用上次目标|导入/ })
    .count();
  const athleteExportButtons = await athletePage
    .getByRole('button', { name: '导出体能测试档案', exact: true })
    .count();
  if (athleteWriteButtons !== 0 || athleteExportButtons !== 1)
    throw new Error(`运动员权限错误：write=${athleteWriteButtons}, export=${athleteExportButtons}`);

  const athleteLoginResponse = await athletePage.request.post(`${baseUrl}/api/auth/login`, {
    data: { username: 'athlete01', password: 'demo123' },
  });
  ...
  const forbiddenWrite = await athletePage.request.post(`${baseUrl}/api/strength-tests`, {
    ...
    data: {
      athleteId: athleteLogin.user.athleteId,
      testDate: '2026-08-22',
      metrics: { verticalJumpCm: 44 },
      targets: {},
      notes: 'permission-check',
    },
  });
  if (forbiddenWrite.status() !== 403)
    throw new Error(`运动员写入接口未拒绝：${forbiddenWrite.status()}`);
  await athletePage
    .locator('.strength-module')
    .screenshot({ path: `${outputDirectory}/运动员只读体能测试档案.png` });
