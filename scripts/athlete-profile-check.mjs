...
try {
  await waitForServer();
  const login = await fetch(`${base}/api/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ username: 'admin01', password: 'demo123' }) });
  ...
  const params = new URLSearchParams({ from: '2026-07-01', to: '2026-07-31', project: athlete.project });
  ...
  assert(wellness.payload.trends.every((trend) => trend.points.every((point) => point.personalValue !== 0 || point.hasPersonalValue)), 'missing wellness was coerced to zero');
  ...
  assert(comparison.payload.comparison.scope.project === athlete.project, 'comparison project scope mismatch');
  assert(comparison.payload.comparison.items.every((item) => item.teamSampleCount === null || item.teamSampleCount >= 2), 'single-athlete team comparison leaked');
  const special = await json(`/api/special-tests?${new URLSearchParams({ ...Object.fromEntries(params), athleteId: String(athlete.id) })}`, token);
  assert(special.status === 200, `athlete special tests request failed: ${special.status}`);
  assert(special.payload.events.every((event) => event.results.every((result) => result.memberAthleteIds.includes(athlete.id))), 'special test filter leaked unrelated crews');
  console.log(JSON.stringify({ athleteId: athlete.id, trendCount: wellness.payload.trends.length, comparisonCount: comparison.payload.comparison.items.length, status: 'ok' }));
} finally {
  server.kill();
