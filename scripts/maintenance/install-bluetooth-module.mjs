...
function ensureReplace(file, transform) {
...
function ensurePageKey(source) {
  ...
  return source.replace(
    /(type\s+PageKey\s*=\s*[^;]*)(;)/,
    (_match, prefix, suffix) => `${prefix} | 'bluetooth'${suffix}`
  );
}

function ensureNavItem(source) {
...
const bluetoothPage = String.raw`import {
  BatteryCharging,
  Bluetooth,
  BluetoothConnected,
  BluetoothOff,
...
write('src/pages/BluetoothConnectPage.tsx', bluetoothPage);
console.log('created src/pages/BluetoothConnectPage.tsx');

ensureReplace('src/components/AppShell.tsx', (source) => {
  let next = ensureLucideIcon(source, 'BluetoothConnected');
  next = ensurePageKey(next);
  next = ensureNavItem(next);
  return next;
});
...
ensureReplace('src/styles.css', (source) => {
