...
for (const label of [
  ...
  if (!(await page.getByText(new RegExp(`^${label}`)).count()))
    throw new Error(`新增表单缺少字段：${label}`);
}
await page.screenshot({ path: `${outputDirectory}/athlete-editor.png`, fullPage: true });
await page.getByRole('button', { name: '关闭', exact: true }).click();

await page
  .locator('.athlete-row-actions')
  .first()
  .getByRole('button', { name: '伤病', exact: true })
  .click();
...
await page
  .locator('.athlete-row-actions')
  .first()
  .getByRole('button', { name: '档案', exact: true })
  .click();
...
await page
  .getByRole('button', { name: '关闭菜单', exact: true })
