...
if (filledMode) {
  ...
  for (let index = 0; index < sourceList.length; index += 1) {
    ...
    profiles.push([athlete.name,'赛艇',group,athlete.gender,`${birthYear}-${String(index % 12 + 1).padStart(2,'0')}-${String(index % 27 + 1).padStart(2,'0')}`,'',...place,'汉族','','','', '', '本科','国家一级',group,'健康','',`${place[0]}${place[1]}`,'','在训',`${startYear}-09-01`, '国家赛艇训练基地','2026年亚运会','冬训','','', '国家体育总局','','','', `来源：国家赛艇队力量素质测试表；组别：${group}`]);
...
dataSheet('运动员信息', ['姓名','运动项目','所属队伍','性别','出生日期','身份证号','省份','城市','区县','民族','手机号','血型','紧急联系人','紧急电话','学历','技术等级','位置号位','身体状态','最好成绩','籍贯','家庭住址','训练状态','开始运动日期','训练场地','备战赛事','备战阶段','集训时间','输送地','输送单位','输送教练','优势项','备注'], '每名运动员一行。姓名、运动项目必填；已有运动员会按此列更新队伍，新运动员须在导入审核区确认队伍。开始运动日期用于训练总览的“运动经验结构”，技术等级也可在“竞技水平评估”表中维护。', { '运动项目':['赛艇','皮划艇','激流'], '性别':['男','女'], '血型':['A','B','AB','O','未知'], '技术等级':['国际级运动健将','运动健将','一级运动员','二级运动员','三级运动员'], '身体状态':['健康','伤病','康复'], '训练状态':['在训','集训','伤停','退役'] });
...
dataSheet('训练课次', ['姓名','日期','课次序号','开始时间','训练类型','训练内容','训练阶段','强度区间','时长分钟','距离千米','RPE','SRPE','SMVL','平均心率','最大心率','平均功率W','桨频SPM'], '每个课次一行；同一运动员同一天用课次序号1、2、3区分。时长、距离至少填写一项；留空表示未填，不等于 0。SRPE留空时系统按已填时长×RPE计算。', { '训练类型':['专项训练','力量训练','恢复训练','测试','比赛'], '训练阶段':['专项训练','体能训练','恢复再生','测试比赛'], '强度区间':['UT3','UT2','UT1','AT','TR','AN'] });
...
dataSheet('测试指标', ['姓名','测试日期','测试类型','指标代码','指标名称','数值','单位','侧别','协议','备注','测试时长分钟'], '通用测试指标：一项一行。相同队伍、同日、同测试类型的各运动员行请填写相同测试时长，训练总览会按一份队伍测试时长累计。FMS和冠军模型请优先使用下方专用工作表。', { '侧别':['center','left','right','bilateral'] });
