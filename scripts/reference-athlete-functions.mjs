...
async function snapshot(name) {
  ...
  audit[name] = {
    ...
    dialogs: await page
      .locator('[role="dialog"]:visible,.el-dialog:visible')
...
async function loginAndOpenAthletes() {
  await page.goto('http://43.139.215.241:7401/#/login', {
...
if (await createCancel.count()) await createCancel.click();
...
if (await editCancel.count()) await editCancel.click();
else await page.keyboard.press('Escape');
...
if (await page.locator('[role="dialog"]:visible,.el-dialog:visible').count()) {
  ...
  if (await injuryCancel.count()) await injuryCancel.click();
  else await page.keyboard.press('Escape');
}
...
if (await cancelDelete.count()) await cancelDelete.click();
else await page.keyboard.press('Escape');
