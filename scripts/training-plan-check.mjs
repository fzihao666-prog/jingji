...
try {
  ...
  if (!plansResult.response.ok || !plansResult.payload?.plans?.length)
    throw new Error('未读取到示例体能训练。');
  ...
  if (overPercentageResult.response.status !== 400)
  ...
  const photoResult = await jsonRequest('/api/athletes/1/photo', coachToken, {
    method: 'POST',
    body: form,
  });
  ...
  const athleteDelete = await jsonRequest(
    `/api/training-plans/${saveResult.payload.id}`,
    athleteToken,
    { method: 'DELETE' }
  );
  ...
  const exportResponse = await fetch(
    `${baseUrl}/api/training-plans/${saveResult.payload.id}/export`,
    {
  ...
  const formula =
    weightCell && typeof weightCell === 'object' && 'formula' in weightCell
      ? weightCell.formula
      : '';
  const result =
    weightCell && typeof weightCell === 'object' && 'result' in weightCell
      ? weightCell.result
      : null;
  if (!formula.includes('$A$7*F7') || result !== 45.5)
    throw new Error('MAX×百分比公式或缓存结果不正确。');
  if (sheet.getCell('H7').value !== 8) throw new Error('实际完成次数未保存为数值。');
  if (workbook.model.media.length !== 1) throw new Error('证件照未嵌入个人导出文件。');
  if (sheet.getCell('E1').value !== '2026.07.28—08.27')
    throw new Error('Excel未显示月度起止日期。');
  ...
  if (
    sheet.getCell('G3').value.includes('雷达口径') ||
    sheet.getCell('G3').value.includes('实际平均重量')
  ) {
  ...
  if (!createHistory.response.ok)
    throw new Error(`历史训练创建失败：${createHistory.payload?.message}`);
  const deleteHistory = await jsonRequest(
    `/api/training-plans/${createHistory.payload.id}`,
    coachToken,
    { method: 'DELETE' }
  );
  if (!deleteHistory.response.ok)
    throw new Error(`历史训练删除失败：${deleteHistory.payload?.message}`);
  const afterDelete = await jsonRequest('/api/training-plans?athleteId=1', coachToken);
  if (afterDelete.payload.plans.some((item) => item.id === createHistory.payload.id))
    throw new Error('历史训练删除后仍可读取。');
  ...
  if (!aiCreate.response.ok || aiCreate.payload.created !== 1)
    throw new Error(`AI生成计划写入失败：${aiCreate.payload?.message}`);
  const generatedResult = await jsonRequest(
    `/api/training-plans?athleteId=${targetId}`,
    coachToken
  );
  const generatedMatrix = generatedResult.payload.plans.find(
    (item) => item.data.startDate === generatedPlan.startDate
  );
  if (!generatedMatrix) throw new Error('AI生成计划保存后未在运动员计划历史中找到矩阵。');
  const generatedExercise = generatedMatrix.data.exercises.find(
    (exercise) => exercise.name === '卧拉'
  );
  if (
  ...
  const editGenerated = await jsonRequest('/api/training-plans', coachToken, {
    ...
    body: JSON.stringify({
      athleteId: targetId,
      planId: generatedMatrix.id,
      data: generatedMatrix.data,
    }),
  });
  if (!editGenerated.response.ok)
    throw new Error(`AI生成矩阵二次编辑保存失败：${editGenerated.payload?.message}`);
  ...
  if (removedImportSave.response.status !== 400)
    throw new Error('已删除的AI识别导入载荷仍可保存。');
