...
export function strengthEvaluation(metrics: StrengthMetricValues, targets: StrengthMetricValues) {
  const comparisons = STRENGTH_METRICS.filter((metric) => metric.targetEnabled && !metric.projects)
    .map((metric) => ({
      ...metric,
  ...
  const gaps = comparisons
    .filter((metric) => (metric.difference ?? 0) < 0)
    .sort((left, right) => (left.difference ?? 0) - (right.difference ?? 0));
  const strengths = [...achieved].sort(
    (left, right) => (right.difference ?? 0) - (left.difference ?? 0)
  );
  const parts: string[] = [];

  if (comparisons.length) {
    parts.push(
      `本次共有${comparisons.length}项指标设置了教练目标，其中${achieved.length}项达到或超过目标。`
    );
    if (strengths.length)
      parts.push(
        `完成较好的指标为${strengths
    ...
    if (gaps.length)
      parts.push(
        `距离目标较大的指标为${gaps
          .slice(0, 3)
          .map((item) => `${item.label}${Math.abs(item.difference ?? 0).toFixed(1)}%`)
          .join('、')}，建议作为下一阶段补强重点。`
      );
  } else {
  ...
  const relativeRows = (
    [
      ...
      ['squatKg', '深蹲'],
      ['deadliftKg', '硬拉'],
    ] as Array<[StrengthMetricKey, string]>
  ).flatMap(([key, label]) => {
