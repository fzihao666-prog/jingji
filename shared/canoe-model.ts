  ...
  latestAdditions: [
    {
      shortLabel: '桨频 / 航速',
      status: '建议采集',
      usage: '用于皮划艇专项技术效率的个人纵向分析',
    },
    {
      shortLabel: '分段配速',
      status: '建议采集',
      usage: '按200米、500米、1000米等距离建立个人基线',
    },
...
export function analyzeCanoePeriod(records: RowingAnalysisRecord[]): RowingPeriodAnalysis {
  ...
  const tracked = training.flatMap((record) => [
    record.rpe,
    record.morningPulse,
    record.weightKg,
    record.sleepHours,
  ...
  if (!alertCount && attentionCount)
    recommendations.push(`持续跟踪${attentionCount}条关注记录，下次训练前完成教练复核。`);
  if (training.length && dataCoverage < 80)
    recommendations.push('RPE、晨脉、睡眠和疲劳数据完整率不足80%，建议补齐后再判断趋势。');
  if (training.length && !alertCount && !attentionCount)
    recommendations.push('维持当前皮划艇训练节奏，继续用统一口径记录距离、强度、桨频和恢复指标。');
  return {
    status,
