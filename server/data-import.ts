...
function parseStandardSheet(matrix: Matrix, sheet: string, project: string, filename: string, defaultDate: string, athletes: ImportAthlete[]) {
  ...
  for (let rowIndex = header.rowIndex + 1; rowIndex < matrix.length; rowIndex += 1) {
    ...
    if (sheet === '竞技水平评估') {
      ...
      const mappings = [['heightCm', '身高cm'], ['weightKg', '体重kg'], ['bodyFatPct', '体脂率%'], ['skeletalMuscleKg', '骨骼肌kg'], ['muscleMassKg', '肌肉量kg'], ['upperLimbMuscleKg', '上肢肌肉kg'], ['lowerLimbMuscleKg', '下肢肌肉kg'], ['trunkMuscleKg', '躯干肌肉kg'], ['subcutaneousFatMm', '皮下脂肪mm'], ['tricepsSkinfoldMm', '肱三头肌皮褶mm'], ['abdominalSkinfoldMm', '腹部皮褶mm'], ['thighSkinfoldMm', '大腿皮褶mm'], ['calfSkinfoldMm', '小腿皮褶mm'], ['visceralFatLevel', '内脏脂肪等级'], ['basalMetabolismKcal', '基础代谢kcal'], ['totalBodyWaterKg', '总水分kg'], ['ecwTbwRatio', '细胞外水比'], ['phaseAngleDeg', '相位角°'], ['visceralFatAreaCm2', '内脏脂肪面积cm²'], ['leftArmLeanKg', '左上肢瘦体重kg'], ['rightArmLeanKg', '右上肢瘦体重kg'], ['trunkLeanKg', '躯干瘦体重kg'], ['leftLegLeanKg', '左下肢瘦体重kg'], ['rightLegLeanKg', '右下肢瘦体重kg']];
...
function upsertBodyItem(item: DataImportItemView, batchId: string, policy: 'skip' | 'update') {
  if (item.metricCode === 'body_composition') {
    ...
    if (existing && policy === 'skip') return { skipped: true, entityId: existing.id };
    const values = ['heightCm', 'weightKg', 'bodyFatPct', 'skeletalMuscleKg', 'muscleMassKg', 'upperLimbMuscleKg', 'lowerLimbMuscleKg', 'trunkMuscleKg', 'subcutaneousFatMm', 'tricepsSkinfoldMm', 'abdominalSkinfoldMm', 'thighSkinfoldMm', 'calfSkinfoldMm', 'visceralFatLevel', 'basalMetabolismKcal', 'totalBodyWaterKg', 'ecwTbwRatio', 'phaseAngleDeg', 'visceralFatAreaCm2', 'leftArmLeanKg', 'rightArmLeanKg', 'trunkLeanKg', 'leftLegLeanKg', 'rightLegLeanKg'].map((key) => numberValue(payload[key]));
